...
static inline constexpr double computeBeta(double stopBandAttenuationDb) {
    ...
    return 0.;
#else
    // c++11 requires exactly one return statement.
    return (stopBandAttenuationDb > 50.) ? (0.1102 * (stopBandAttenuationDb - 8.7)) :
        ((stopBandAttenuationDb - 21.) > 0.) ? (0.5842 * pow(stopBandAttenuationDb - 21., 0.4) + 0.07886 * (stopBandAttenuationDb - 21.)) : 0.;
#endif
}
