...
int audio_dec_init(audio_decoder_operations_t *adec_ops)
{
    ...
    audio_codec_print("[%s %d]gFaadCxt.b.buffer: space/%di\n",__FUNCTION__,__LINE__,FAAD_MIN_STREAMSIZE*MAX_CHANNELS);
    
    adec_ops->nInBufSize=768*adec_ops->channels;
    audio_codec_print("[%s %d] nInBufSize %d\n", __FUNCTION__,__LINE__,adec_ops->nInBufSize);
