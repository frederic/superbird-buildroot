...
int alsa_get_audio_port(char *str)
{
    int port = -1, err = 0;
    int fd = -1;
    unsigned fileSize = 512;
    ...
    if(0 == strncmp(str, "SPDIF", 5)){
        pd = strcasestr(read_buf, "SPDIF");
        if(pd){
            port = *(pd - 3) - '0';
            adec_print("%s  \n", (pd - 3));
        }
    }
    if(!pd && (0 == strncmp(str, "I2S", 3))){
        pd = strcasestr(read_buf, "I2S");
        if (pd) {
            port = *(pd - 24) - '0';
            adec_print("%s  \n", (pd - 24));
        }
...
static size_t pcm_write_raw(alsa_param_t * alsa_param, u_char * data, size_t count)
{
    ...
    */
    while (count > 0) {
        ...
        if (r > 0) {
#if 0
            fp = fopen("/tmp/eac3file_2.wav", "a+");
            if (fp) {
                fwrite((char *)data, 1, r*alsa_param->bits_per_frame/8, fp);
                fclose(fp);
            }
#endif
            result += r;
...
static void *alsa_playback_raw_loop(void *args)
{
 ...
 /*   pthread_mutex_init(&alsa_params->playback_mutex, NULL);
    ...
    while (!alsa_params->stop_flag) {
        ...
        audec->raw_bytes_readed+=len;

        while (alsa_params->pause_flag) {
            usleep(10000);
        }
#if 0
        fp = fopen("/tmp/eac3file.wav", "a+");
        if (fp) {
                fwrite((char *)(buffer + offset), 1, len, fp);
                fclose(fp);
        }
#endif
...
int alsa_init_raw(struct aml_audio_dec* audec)
{
    ...
    sound_dev_id = alsa_get_audio_port("SPDIF");
    if (sound_dev_id < 0) {
        sound_dev_id = 0;
