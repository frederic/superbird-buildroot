...
static void *alsa_playback_loop(void *args)
{
 ...
 /*   pthread_mutex_init(&alsa_params->playback_mutex, NULL);
    ...
    adec_print("alsa playback loop start to run !, alsa_params->stop_flag = %d\n", alsa_params->stop_flag);
...
int alsa_init(struct aml_audio_dec* audec)
{
   ...
   alsa_param->wait_flag=0;
    ...
    if(((AUDIO_SPDIF_PASSTHROUGH == dgraw)||(AUDIO_HDMI_PASSTHROUGH == dgraw))&& \
        ...
        sound_dev_id = alsa_get_audio_port("I2S");
        if (sound_dev_id < 0) {
            sound_dev_id = 0;
...
int alsa_resume(struct aml_audio_dec* audec)
{
    ...
    int res = 0;
    alsa_param_t *alsa_params = (alsa_param_t *)audec->aout_ops.private_data;

    if (!alsa_params->started) {
        return alsa_start(audec);
    }
    ...
    adec_print("alsa out resume - end\n");
    return res;
}
...
unsigned long alsa_latency(struct aml_audio_dec* audec)
{
    ...
    adec_print("alsa_param->buffer_size :%d buffered_data:%d, sample_num=%d\n",alsa_param->buffer_size,buffered_data, sample_num);
...
int ret = snd_pcm_delay(alsa_param->handle,&sample_num);
    ...
    printf("alsa_param->buffer_size :%d buffered_data:%d, sample_num=%d, bits_per_sample = %d\n",alsa_param->buffer_size,buffered_data, sample_num, alsa_param->bits_per_sample);
