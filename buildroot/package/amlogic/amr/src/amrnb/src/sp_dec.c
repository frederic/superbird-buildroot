      ...
      Word32 *Az_4 )
{
    ...
    * Post filtering
    */
   memcpy( syn_work, syn, sizeof( Word32 )*L_FRAME );
   Az = Az_4;
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      memcpy( h, Ap3, sizeof( Word32 )*MP1 );
      memset( &h[M +1], 0, sizeof( Word32 )*( 22 - M - 1 ) );
      ...
      *p1 = *p1 - ( ( temp2 * st->preemph_state_mem_pre ) >> 15 );
      ...
      if (overflow){
         ...
         memcpy(st->mem_syn_pst, &syn[i_subfr + 30], sizeof( Word32 )*M);
      }
   ...
   memcpy( &syn_work[- M], &syn_work[L_FRAME - M], sizeof( Word32 )*M );
   return;
}
...
static int Post_Filter_reset( Post_FilterState *state )
{
   ...
   memset( state->mem_syn_pst, 0, sizeof( Word32 )*M );
   memset( state->res2, 0, sizeof( Word32 )*L_SUBFR );
   memset( state->synth_buf, 0, sizeof( Word32 )*( L_FRAME + M ) );
   return 0;
}
