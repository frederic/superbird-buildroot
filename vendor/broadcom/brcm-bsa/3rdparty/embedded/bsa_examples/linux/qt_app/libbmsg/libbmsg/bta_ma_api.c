...
**
** Description      Set the bmessage-body-charset-property value for the specified
**                  bMessage body.
**
** Parameters       p_body - Pointer to a bMessage body
**                  charset - Charset
...
**
** Description      Get the first text string from the specified message content.
**
** Parameters       p_content - Pointer to a message content
**
...
**
** Description      Add a property to the specified vCard.
**
** Parameters       p_vcard - Pointer to a vCard
...
**
*******************************************************************************/
tBTA_MA_STATUS BTA_MaBuildMapBmsgObj(tBTA_MA_BMSG *p_msg, tBTA_MA_STREAM *p_stream)
{
...
**
*******************************************************************************/
tBTA_MA_STATUS BTA_MaParseMapBmsgObj(tBTA_MA_BMSG *p_msg, tBTA_MA_STREAM *p_stream)
{
    ...
    if ( p_msg && p_stream )
    {
             ...
             && (strcmp(sz, "BMSG") == 0) )
        {
            while ( bta_ma_get_tag(p_stream, sz, BTA_MA_MAX_SIZE) )
            {
                ...
                {
                    bta_ma_get_value(p_stream, sz, BTA_MA_MAX_SIZE);
                    APPL_TRACE_WARNING1("bta_ma_parse_map_bmsg_obj - Unknown tag: '%s'", sz);
                }
            }
        }
    }
...
**
*******************************************************************************/
BOOLEAN BTA_MaInitMemStream(tBTA_MA_STREAM *p_stream, UINT8 *p_buffer, UINT16 size)
{
    if ( !p_stream )
        return(FALSE);
...
**
*******************************************************************************/
BOOLEAN BTA_MaInitFileStream(tBTA_MA_STREAM *p_stream, const char *p_path, int oflags)
{
    if ( !p_stream )
    {
