 ...
 **Name:      app_av.c
 **
 **Description: bluetooth app audio & video source streaming
 **
 **Copyright(c)  Amlogic
 **
 ****************************************************************************************/
...
#include "app_alsa.h"
#endif
#define APP_AV_AUTOPLAY APP_AV_PLAYTYPE_FIFO
...
#define APP_AV_MAX_AUDIO_BUF 512
...
#define APP_AV_FIFO_NAME "/tmp/fifo"
...
#define APP_AV_PLAYTYPE_FIFO    5  /* play fifo data */
...
void app_av_cback(tBSA_AV_EVT event, tBSA_AV_MSG *p_data)
{
        ...
        else
        {
            if (p_data->open.status == BSA_SUCCESS)
            {
                ...
                if (app_av_cb.play_state == APP_AV_PLAY_STOPPED)
                {
                    ...
                    app_av_play_mic();
#endif
#if (APP_AV_AUTOPLAY==APP_AV_PLAYTYPE_FIFO)
                    /*Play fifo */
                    APP_INFO0("Start Playing fifo");
                    app_av_play_fifo();
#endif
    ...
    case BSA_AV_START_EVT:
        ...
        system("pacmd set-sink-mute 0 1");
    ...
    case BSA_AV_STOP_EVT:
        ...
        system("pacmd set-sink-mute 0 0");
        break;
...
int app_av_read_fifo(short *pOut, int len){
    int fd;
    int r_bytes;
    fd = open(APP_AV_FIFO_NAME, O_RDONLY);
    r_bytes = read(fd, pOut, len);
    close(fd);
    return r_bytes;
}
...
int app_av_play_fifo(void){
    ...
    start_param.media_feeding.cfg.pcm.bit_per_sample = 16;
    start_param.feeding_mode = app_av_cb.uipc_cfg.is_blocking? BSA_AV_FEEDING_ASYNCHRONOUS: BSA_AV_FEEDING_SYNCHRONOUS;
    start_param.latency = app_av_cb.uipc_cfg.period/1000;
    start_param.cp_id = app_av_cb.cp_id;
    start_param.scmst_flag = app_av_cb.cp_scms_flag;
    app_av_cb.play_type = APP_AV_PLAYTYPE_FIFO;
    app_av_cb.play_list = FALSE;
...
static void app_uipc_tx_thread(void)
{
    ...
    while (1)
    {
        ...
        if (app_av_cb.play_type == APP_AV_PLAYTYPE_FIFO)
        {
            APP_DEBUG0("Playing fifo");
        }
        ...
        do
        {
            ...
            case APP_AV_PLAYTYPE_FILE:
                ...
                break;
            case APP_AV_PLAYTYPE_FIFO:
                nb_bytes = app_av_read_fifo(app_av_cb.audio_buf, app_av_cb.uipc_cfg.length);
              //  APP_ERROR1("GET DATA LEN IS %d",nb_bytes);
                break;
#ifdef PCM_ALSA
            case APP_AV_PLAYTYPE_MIC:
