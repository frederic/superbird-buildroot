...
void app_ble_server_profile_cback(tBSA_BLE_EVT event,  tBSA_BLE_MSG *p_data)
{
    ...
    case BSA_BLE_SE_READ_EVT:
        ...
        send_server_resp.len = (UINT16) sizeof(attribute_value) - p_data->ser_read.offset;
        send_server_resp.auth_req = GATT_AUTH_REQ_NONE;
        memcpy(send_server_resp.value, attribute_value + p_data->ser_read.offset, BSA_BLE_MAX_ATTR_LEN);
