...
bool AmlAVCodec::AVWriteSample(const scoped_refptr<InputBuffer> &input_buffer,
  ...
  if(isvideo)
  {
    //youtube 20 require to start after 0.5 second data, video frame count depends on fps
  ...
  if (isvideo) {
    ...
    if (num_frame_pts == 0) {
      ...
      if (((sinfo.color_metadata.transfer == kSbMediaTransferIdSmpteSt2084) ||
...
AmlAudioRenderer::AmlAudioRenderer(SbMediaAudioCodec audio_codec,
  ...
  CLOG(WARNING) << "audio format_tag:" << audio_sample_info.format_tag
              ...
              << " average_bytes_per_second:" << audio_sample_info.average_bytes_per_second
              << " block_alignment:" << audio_sample_info.block_alignment
              << " bits_per_sample:" << audio_sample_info.bits_per_sample
  ...
  if (audio_codec == kSbMediaAudioCodecAac) {
    ...
    //codec_param->audio_info.sample_rate = audio_sample_info.samples_per_second;
    //codec_param->audio_info.channels = audio_sample_info.number_of_channels;
    //codec_param->audio_info.codec_id = 0x4f505553;  // OPUS
    codec_param->audio_channels = audio_sample_info.number_of_channels;
    codec_param->audio_samplerate = audio_sample_info.samples_per_second;
  ...
  codec_param->audio_info.sample_rate = audio_sample_info.samples_per_second;
  codec_param->audio_info.channels = audio_sample_info.number_of_channels;
  //codec_param->audio_info.bitrate = audio_sample_info.samples_per_second * audio_sample_info.bits_per_sample;
  //codec_param->audio_info.block_align = audio_sample_info.block_alignment;
  codec_param->audio_info.extradata_size = audio_sample_info.audio_specific_config_size;
  SbMemoryCopy(codec_param->audio_info.extradata, audio_sample_info.audio_specific_config, audio_sample_info.audio_specific_config_size);
