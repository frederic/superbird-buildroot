...
SbBlitterSwapChain SbBlitterCreateSwapChainFromWindow(SbBlitterDevice device,
  ...
  primary_dsc.flags = static_cast<DFBSurfaceDescriptionFlags>(DSDESC_CAPS | DSDESC_PIXELFORMAT);
  primary_dsc.caps = static_cast<DFBSurfaceCapabilities>(
      DSCAPS_PRIMARY | DSCAPS_FLIPPING | DSCAPS_STATIC_ALLOC | DSCAPS_TRIPLE);
  primary_dsc.pixelformat = DSPF_ARGB;
