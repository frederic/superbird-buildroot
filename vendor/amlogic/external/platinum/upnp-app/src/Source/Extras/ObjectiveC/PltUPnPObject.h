...
( PLT_Action * )_action;
- ( NPT_Result )setValue:
( NSString * )value forArgument:
( NSString * )argument;
- ( NPT_Result )setErrorCode:
( unsigned int )code withDescription:
...
+---------------------------------------------------------------------*/
@interface PLT_DeviceHostObject :
NSObject {
    @private
...
- ( void )setDevice:
( PLT_DeviceHostReference * )_device;
