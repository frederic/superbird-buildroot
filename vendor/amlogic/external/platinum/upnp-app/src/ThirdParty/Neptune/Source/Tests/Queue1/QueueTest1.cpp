...
Test1()
{
    ...
    WriterThread writer1 ( queue, "1" );
    writer1.Start();
    NPT_System::Sleep ( NPT_TimeInterval ( 0.3f ) );
    NPT_Debug ( "creating writer 2\n" );
    WriterThread writer2 ( queue, "2" );
    writer2.Start();
    NPT_System::Sleep ( NPT_TimeInterval ( 0.3f ) );
    // create 4 readers
    NPT_Debug ( "creating reader 1\n" );
    ReaderThread reader1 ( queue, "1", NPT_TimeInterval ( 0.0f ) );
    reader1.Start();
    NPT_System::Sleep ( NPT_TimeInterval ( 0.3f ) );
    NPT_Debug ( "creating reader 2\n" );
    ReaderThread reader2 ( queue, "2", NPT_TimeInterval ( 0.0f ) );
    reader2.Start();
    NPT_System::Sleep ( NPT_TimeInterval ( 0.3f ) );
    NPT_Debug ( "creating reader 3\n" );
    ReaderThread reader3 ( queue, "3", NPT_TimeInterval ( 0.0f ) );
    reader3.Start();
    NPT_System::Sleep ( NPT_TimeInterval ( 0.3f ) );
    NPT_Debug ( "creating reader 4\n" );
    ReaderThread reader4 ( queue, "4", NPT_TimeInterval ( 0.0f ) );
    reader4.Start();
    NPT_System::Sleep ( NPT_TimeInterval ( 0.3f ) );
...
Test2()
{
    ...
    // create 2 readers
    NPT_Debug ( "creating reader 1\n" );
    ReaderThread reader1 ( queue, "1", NPT_TimeInterval ( 0.05f ) );
    reader1.Start();
    NPT_Debug ( "creating reader 2\n" );
    ReaderThread reader2 ( queue, "2", NPT_TimeInterval ( 0.065f ) );
    reader2.Start();
    for ( int i = 0; i < 30; i++ )
    {
