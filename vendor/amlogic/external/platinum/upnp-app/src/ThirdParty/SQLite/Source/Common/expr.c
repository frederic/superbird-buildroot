...
static char comparisonAffinity(Expr *pExpr)
{
    ...
    assert(pExpr->op == TK_EQ || pExpr->op == TK_IN || pExpr->op == TK_LT ||
        pExpr->op == TK_GT || pExpr->op == TK_GE || pExpr->op == TK_LE || pExpr
        ->op == TK_NE);
    assert(pExpr->pLeft);
 ...
 */
static int codeCompare(Parse *pParse,  /* The parsing (and code generating)
    context */
...
)
{
    ...
    return sqlite3VdbeOp3(pParse->pVdbe, opcode, p1, dest, (void*)p3,
        P3_COLLSEQ);
}
...
void sqlite3ExprAssignVarNumber(Parse *pParse, Expr *pExpr)
{
    ...
    else
    {
        ...
        {
            Expr *pE;
            if ((pE = pParse->apVarExpr[i]) != 0 && pE->token.n == n && memcmp
                (pE->token.z, pToken->z, n) == 0)
            {
        ...
        if (i >= pParse->nVarExpr)
        {
            ...
            if (pParse->nVarExpr >= pParse->nVarExprAlloc - 1)
            {
                ...
                sqliteReallocOrFree((void **) &pParse->apVarExpr, pParse
                    ->nVarExprAlloc *sizeof(pParse->apVarExpr[0]));
            }
...
void sqlite3ExprDelete(Expr *p)
{
    if (p == 0)
        return ;
    if (p->span.dyn)
        sqliteFree((char*)p->span.z);
    if (p->token.dyn)
        sqliteFree((char*)p->token.z);
 ...
 */
     ...
     || !defined(SQLITE_OMIT_SUBQUERY)SrcList *sqlite3SrcListDup(SrcList *p)
    {
        SrcList *pNew;
        int i;
        int nByte;
 ...
 */
static int lookupName(Parse *pParse,  /* The parsing context */
...
)
{
    ...
    while (pNC && cnt == 0)
    {
        ...
        if (pSrcList)
        {
            for (i = 0, pItem = pSrcList->a; i < pSrcList->nSrc; i++, pItem++)
            {
                ...
                if (zTab)
                {
                    ...
                    else
                    {
                        ...
                        if (zTabName == 0 || sqlite3StrICmp(zTabName, zTab) !=
                            0)
                            continue;
                        if (zDb != 0 && sqlite3StrICmp(db->aDb[iDb].zName, zDb)
                            != 0)
                        {
                            continue;
                        }
                ...
                for (j = 0, pCol = pTab->aCol; j < pTab->nCol; j++, pCol++)
                {
                    if (sqlite3StrICmp(pCol->zName, zCol) == 0)
                    {
                        ...
                        pExpr->pColl = sqlite3FindCollSeq(db, ENC(db), zColl,
                            - 1, 0);
                        ...
                        if ((pUsing = pItem->pUsing) != 0)
                        {
                            ...
                            {
                                if (sqlite3StrICmp(pUsing->a[k].zName, zCol) ==
                                    0)
                                {
                                    pItem++;
                                    i++;
            ...
            if (zDb == 0 && zTab != 0 && cnt == 0 && pParse->trigStack != 0)
            {
                ...
                Table *pTab = 0;
                if (pTriggerStack->newIdx !=  - 1 && sqlite3StrICmp("new", zTab)
                    == 0)
                {
                ...
                else if (pTriggerStack->oldIdx !=  - 1 && sqlite3StrICmp("old",
                    zTab) == 0)
                {
                ...
                if (pTab)
                {
                    ...
                    for (iCol = 0; iCol < pTab->nCol; iCol++, pCol++)
                    {
                        if (sqlite3StrICmp(pCol->zName, zCol) == 0)
                        {
                            ...
                            pExpr->pColl = sqlite3FindCollSeq(db, ENC(db),
                                zColl,  - 1, 0);
                            pExpr->pTab = pTab;
                            break;
                        }
    ...
    pExpr->pRight = 0;
    pExpr->op = TK_COLUMN;
    lookupname_end_2: sqliteFree(zCol);
    if (cnt == 1)
    {
        assert(pNC != 0);
...
static int nameResolverStep(void *pArg, Expr *pExpr)
{
    ...
    ExprSetProperty(pExpr, EP_Resolved);
        ...
        if (pNC->pSrcList && pNC->pSrcList->nAlloc > 0)
        {
            ...
            {
                assert(pSrcList->a[i].iCursor >= 0 && pSrcList->a[i].iCursor <
                    pParse->nTab);
            }
        }
    #endif
        ...
        case TK_FUNCTION:
            {
                ...
                {
                    sqlite3ErrorMsg(pParse,
                        "misuse of aggregate function %.*s()", nId, zId);
                    pNC->nErr++;
                    is_agg = 0;
                }
 ...
 */
int sqlite3ExprResolveNames(NameContext *pNC,  /* Namespace to resolve
    expressions in. */
            ...
            case TK_IN:
                {
                    ...
                    if (pExpr->pSelect)
                    {
                        ...
                        sqlite3Select(pParse, pExpr->pSelect, SRT_Set, iParm, 0,
                            0, 0, 0);
                        pEList = pExpr->pSelect->pEList;
                        if (pEList && pEList->nExpr > 0)
                        {
                            keyInfo.aColl[0] = binaryCompareCollSeq(pParse,
                                pExpr->pLeft, pEList->a[0].pExpr);
                        }
                    }
                    else if (pExpr->pList)
                    {
                        ...
                        for (i = pList->nExpr, pItem = pList->a; i > 0; i--,
                            pItem++)
                        {
...
void sqlite3ExprCode(Parse *pParse, Expr *pExpr)
{
        ...
        case TK_AGG_COLUMN:
            {
                ...
                {
                    sqlite3VdbeAddOp(v, OP_Column, pAggInfo->sortingIdx, pCol
                        ->iSorterColumn);
                    break;
                }
        ...
        case TK_COLUMN:
            {
                if (pExpr->iTable < 0)
                {
                    ...
                    sqlite3VdbeAddOp(v, OP_Dup, pParse->ckOffset - pExpr
                        ->iColumn - 1, 1);
                }
                else if (pExpr->iColumn >= 0)
                {
                    ...
                    sqlite3ColumnDefault(v, pTab, iCol);
                        ...
                        if (pTab && pTab->aCol[iCol].affinity ==
                            SQLITE_AFF_REAL)
                        {
        ...
        case TK_STRING:
            {
                ...
                sqlite3VdbeOp3(v, op, 0, 0, (char*)pExpr->token.z, pExpr
                    ->token.n);
                break;
            }
        case TK_NULL:
        ...
        case TK_VARIABLE:
            {
                ...
                if (pExpr->token.n > 1)
                {
                    sqlite3VdbeChangeP3(v,  - 1, (char*)pExpr->token.z, pExpr
                        ->token.n);
                }
                break;
            }
        ...
        case TK_AGG_FUNCTION:
            {
                ...
                {
                    sqlite3VdbeAddOp(v, OP_MemLoad, pInfo->aFunc[pExpr
                        ->iAgg].iMem, 0);
                }
                break;
            }
        ...
        case TK_FUNCTION:
            {
                ...
                if (pDef->needCollSeq)
                {
                    ...
                    sqlite3VdbeOp3(v, OP_CollSeq, 0, 0, (char*)pColl,
                        P3_COLLSEQ);
                }
                sqlite3VdbeOp3(v, OP_Function, constMask, nExpr, (char*)pDef,
                    P3_FUNCDEF);
                stackChng = 1-nExpr;
                break;
            }
            ...
            case TK_IN:
                {
                    ...
                    sqlite3VdbeAddOp(v, OP_NotNull,  - 1, addr + 4); /* addr +
                        0 */
        ...
        case TK_CASE:
            {
                ...
                for (i = 0; i < nExpr; i = i + 2)
                {
                    ...
                    if (pExpr->pLeft)
                    {
                        ...
                        jumpInst = codeCompare(pParse, pExpr->pLeft,
                            aListelem[i].pExpr, OP_Ne, 0, 1);
            ...
            case TK_RAISE:
                {
                    ...
                    {
                        assert(pExpr->iColumn == OE_Rollback || pExpr->iColumn
                            == OE_Abort || pExpr->iColumn == OE_Fail);
                        sqlite3DequoteExpr(pExpr);
                        sqlite3VdbeOp3(v, OP_Halt, SQLITE_CONSTRAINT, pExpr
                            ->iColumn, (char*)pExpr->token.z, pExpr->token.n);
                    }
                    else
                    {
                        assert(pExpr->iColumn == OE_Ignore);
                        sqlite3VdbeAddOp(v, OP_ContextPop, 0, 0);
                        sqlite3VdbeAddOp(v, OP_Goto, 0, pParse->trigStack
                            ->ignoreJump);
    ...
    void sqlite3ExprCodeAndCache(Parse *pParse, Expr *pExpr)
    {
        ...
        if (addr2 > addr1 + 1 || sqlite3VdbeGetOp(v, addr1)->opcode ==
            OP_Function)
        {
 ...
 */
int sqlite3ExprCodeExprList(Parse *pParse,  /* Parsing context */
        ...
        case TK_EQ:
            {
                ...
                sqlite3ExprCode(pParse, pExpr->pLeft);
                sqlite3ExprCode(pParse, pExpr->pRight);
                codeCompare(pParse, pExpr->pLeft, pExpr->pRight, op, dest,
                    jumpIfNull);
                break;
            }
        case TK_ISNULL:
...
int sqlite3ExprCompare(Expr *pA, Expr *pB)
{
    ...
    if (pA->pList)
    {
        ...
        {
            if (!sqlite3ExprCompare(pA->pList->a[i].pExpr, pB->pList
                ->a[i].pExpr))
            {
                return 0;
            }
    ...
    if (pA->token.z)
    {
        ...
        if (pB->token.n != pA->token.n)
            return 0;
        if (sqlite3StrNICmp((char*)pA->token.z, (char*)pB->token.z, pB->token.n)
            != 0)
        {
            return 0;
        }
...
{
    int i;
    i = sqlite3ArrayAllocate((void **) &pInfo->aFunc, sizeof(pInfo->aFunc[0]),
        2);
    if (i < 0)
    {
        return  - 1;
                ...
                if (pSrcList)
                {
                    ...
                    for (i = 0; i < pSrcList->nSrc; i++, pItem++)
                    {
                        ...
                        if (pExpr->iTable == pItem->iCursor)
                        {
                            ...
                            {
                                if (pCol->iTable == pExpr->iTable && pCol
                                    ->iColumn == pExpr->iColumn)
                                {
                                    break;
                                }
                            }
                            if (i >= pAggInfo->nColumn && (i = addAggInfoColumn
                                (pAggInfo)) >= 0)
                            {
                                ...
                                if (pAggInfo->pGroupBy)
                                {
                                    ...
                                    {
                                        Expr *pE = pTerm->pExpr;
                                        if (pE->op == TK_COLUMN && pE->iTable
                                            == pExpr->iTable && pE->iColumn ==
                                            pExpr->iColumn)
                                        {
                    ...
                    if (i >= pAggInfo->nFunc)
                    {
                        ...
                        if (i >= 0)
                        {
                            ...
                            pItem->pFunc = sqlite3FindFunction(pParse->db,
                                (char*)pExpr->token.z, pExpr->token.n, pExpr
                                ->pList ? pExpr->pList->nExpr: 0, enc, 0);
