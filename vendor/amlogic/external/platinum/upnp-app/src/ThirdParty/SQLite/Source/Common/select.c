 ...
 ** structure.
 */
Select *sqlite3SelectNew(ExprList *pEList,  /* which columns to include in the
    result */
...
int sqlite3JoinType(Parse *pParse, Token *pA, Token *pB, Token *pC)
{
    ...
    for (i = 0; i < 3 && apAll[i]; i++)
    {
        ...
        {
            if (p->n == keywords[j].nChar && sqlite3StrNICmp((char*)p->z,
                keywords[j].zKeyword, p->n) == 0)
            {
    ...
    if ((jointype &(JT_INNER | JT_OUTER)) == (JT_INNER | JT_OUTER) || (jointype
        &JT_ERROR) != 0)
    {
        const char *zSp1 = " ";
        const char *zSp2 = " ";
 ...
 */
static void addWhereTerm(const char *zCol,  /* Name of the column */
    ...
    for (i = 0; i < pSrc->nSrc - 1; i++, pRight++, pLeft++)
    {
        ...
        if (pLeft->jointype &JT_NATURAL)
        {
            ...
            } for (j = 0; j < pLeftTab->nCol; j++)
            {
                ...
                {
                    addWhereTerm(zName, pLeftTab, pLeft->zAlias, pRightTab,
                        pRight->zAlias, pRight->iCursor, &p->pWhere);
                }
            }
        }
        ...
        if (pLeft->pUsing)
        {
            ...
            for (j = 0; j < pList->nId; j++)
            {
                ...
                if (columnIndex(pLeftTab, zName) < 0 || columnIndex(pRightTab,
                    zName) < 0)
                {
                    sqlite3ErrorMsg(pParse,
                ...
                addWhereTerm(zName, pLeftTab, pLeft->zAlias, pRightTab, pRight
                    ->zAlias, pRight->iCursor, &p->pWhere);
            }
        }
    }
    return 0;
 ...
 */
static void pushOntoSorter(Parse *pParse,  /* Parser context */
ExprList *pOrderBy,  /* The ORDER BY clause */
 ...
 */
static void codeOffset(Vdbe *v,  /* Generate code into this VM */
 ...
 */
static void codeDistinct(Vdbe *v,  /* Generate code into this VM */
 ...
 */
static int selectInnerLoop(Parse *pParse,  /* The parser context */
            ...
            case SRT_Set:
                {
                    ...
                    else
                    {
                        ...
                        affinity = sqlite3CompareAffinity(pEList->a[0].pExpr,
                            affinity);
                        sqlite3VdbeOp3(v, OP_MakeRecord, 1, 0, &affinity, 1);
                        sqlite3VdbeAddOp(v, OP_IdxInsert, (iParm &0x0000FFFF),
                            0);
                    }
                    sqlite3VdbeJumpHere(v, addr2);
                    break;
 ...
 */
static void generateSortTail(Parse *pParse,  /* Parsing context */
    ...
    switch (eDest)
    {
        ...
        case SRT_VirtualTab:
            ...
            case SRT_Set:
                {
                    assert(nColumn == 1);
                    sqlite3VdbeAddOp(v, OP_NotNull,  - 1,
                        sqlite3VdbeCurrentAddr(v) + 3);
                    sqlite3VdbeAddOp(v, OP_Pop, 1, 0);
                    sqlite3VdbeAddOp(v, OP_Goto, 0, sqlite3VdbeCurrentAddr(v) +
                        3);
 ...
 */
static const char *columnType(NameContext *pNC, Expr *pExpr, const char
    **pzOriginDb, const char **pzOriginTab, const char **pzOriginCol)
{
    char const *zType = 0;
        ...
        case TK_COLUMN:
            {
                ...
                while (pNC && !pTab)
                {
                    ...
                    for (j = 0; j < pTabList->nSrc && pTabList->a[j].iCursor !=
                        pExpr->iTable; j++)
                        ;
                    if (j < pTabList->nSrc)
                    {
                ...
                if (pTab->pSchema)
                {
                    ...
                    if (pNC->pParse)
                    {
                        int iDb = sqlite3SchemaToIndex(pNC->pParse->db, pTab
                            ->pSchema);
 ...
 ** in the result set.
 */
static void generateColumnTypes(Parse *pParse,  /* Parser context */
 ...
 */
static void generateColumnNames(Parse *pParse,  /* Parser context */
SrcList *pTabList,  /* List of tables */
ExprList *pEList /* Expressions defining the result set */
)
{
    ...
    for (i = 0; i < pEList->nExpr; i++)
    {
        ...
        if (p->op == TK_COLUMN && pTabList)
        {
            ...
            char *zCol;
            int iCol = p->iColumn;
            for (j = 0; j < pTabList->nSrc && pTabList->a[j].iCursor != p
                ->iTable; j++) {}
            ...
            {
                sqlite3VdbeSetColName(v, i, COLNAME_NAME, (char*)p->span.z, p
                    ->span.n);
            }
...
Table *sqlite3ResultSetOfSelect(Parse *pParse, char *zTabName, Select *pSelect)
{
    ...
    for (i = 0, pCol = aCol; i < pTab->nCol; i++, pCol++)
    {
         ...
         */
        p = pEList->a[i].pExpr;
        assert(p->pRight == 0 || p->pRight->token.z == 0 || p->pRight
            ->token.z[0] != 0);
        ...
        else if (p->op == TK_DOT && (pR = p->pRight) != 0 && pR->token.z && pR
            ->token.z[0])
        {
...
static int prepSelectStmt(Parse *pParse, Select *p)
{
    ...
    for (i = 0, pFrom = pTabList->a; i < pTabList->nSrc; i++, pFrom++)
    {
        ...
        if (pFrom->zName == 0)
        {
                ...
                if (pFrom->zAlias == 0)
                {
                    pFrom->zAlias = sqlite3MPrintf("sqlite_subquery_%p_",
                        (void*)pFrom->pSelect);
                }
                assert(pFrom->pTab == 0);
                pFrom->pTab = pTab = sqlite3ResultSetOfSelect(pParse, pFrom
                    ->zAlias, pFrom->pSelect);
                if (pTab == 0)
                {
                    return 1;
                }
        ...
        else
        {
            ...
            assert(pFrom->pTab == 0);
            pFrom->pTab = pTab = sqlite3LocateTable(pParse, pFrom->zName, pFrom
                ->zDatabase);
            if (pTab == 0)
            {
                return 1;
    ...
    for (k = 0; k < pEList->nExpr; k++)
    {
        ...
        if (pE->op == TK_ALL)
            break;
        if (pE->op == TK_DOT && pE->pRight && pE->pRight->op == TK_ALL && pE
            ->pLeft && pE->pLeft->op == TK_ID)
            break;
    }
    rc = 0;
    if (k < pEList->nExpr)
    {
        ...
        int longNames = (flags &SQLITE_FullColNames) != 0 && (flags
            &SQLITE_ShortColNames) == 0;
        for (k = 0; k < pEList->nExpr; k++)
        {
            Expr *pE = a[k].pExpr;
            if (pE->op != TK_ALL && (pE->op != TK_DOT || pE->pRight == 0 || pE
                ->pRight->op != TK_ALL))
            {
                    ...
                    ++)
                {
                    ...
                    for (j = 0; j < pTab->nCol; j++)
                    {
                        ...
                        if (zTabName && (longNames || pTabList->nSrc > 1))
                        {
                            ...
                            setToken(&pExpr->span, sqlite3MPrintf("%s.%s",
                                zTabName, zName));
                            pExpr->span.dyn = 1;
                            pExpr->token.z = 0;
                            pExpr->token.n = 0;
                        ...
                        if (longNames)
                        {
                            pNew = sqlite3ExprListAppend(pNew, pExpr, &pExpr
                                ->span);
                        }
                        else
                        {
                            pNew = sqlite3ExprListAppend(pNew, pExpr, &pRight
                                ->token);
                        }
                    }
                }
     ...
     */
    static int matchOrderbyToColumn(Parse *pParse,  /* A place to leave error
        messages */
    ...
    )
    {
        ...
        if (pSelect->pPrior)
        {
            if (matchOrderbyToColumn(pParse, pSelect->pPrior, pOrderBy, iTable,
                0))
            {
                return 1;
            }
        ...
        for (i = 0; i < pOrderBy->nExpr; i++)
        {
            ...
            if (sqlite3ExprIsInteger(pE, &iCol))
            {
                ...
                {
                    sqlite3ErrorMsg(pParse,
                        "ORDER BY position %d should be between 1 and %d", iCol,
                        pEList->nExpr);
                    nErr++;
                    break;
                }
            ...
            {
                if (pEList->a[j].zName && (pE->op == TK_ID || pE->op ==
                    TK_STRING))
                {
                    char *zName,  *zLabel;
...
static void createSortingIndex(Parse *pParse, Select *p, ExprList *pOrderBy)
{
    if (pOrderBy)
    {
        ...
        addr = sqlite3VdbeAddOp(pParse->pVdbe, OP_OpenVirtual, pOrderBy
            ->iECursor, pOrderBy->nExpr + 1);
    ...
    )
    {
        ...
        {
            sqlite3ErrorMsg(pParse,
                "ORDER BY clause should come after %s not before", selectOpName
                (p->op));
            rc = 1;
            goto multi_select_end;
        }
        ...
        {
            sqlite3ErrorMsg(pParse,
                "LIMIT clause should come after %s not before", selectOpName(p
                ->op));
            rc = 1;
            goto multi_select_end;
        }
        ...
        switch (p->op)
        {
            case TK_ALL:
                {
                    if (pOrderBy == 0)
                    {
                        ...
                        rc = sqlite3Select(pParse, pPrior, eDest, iParm, 0, 0,
                            0, aff);
                        p->pLimit = 0;
                        p->pOffset = 0;
                        if (rc)
                        ...
                        if (p->iLimit >= 0)
                        {
                            addr = sqlite3VdbeAddOp(v, OP_IfMemZero, p->iLimit,
                                0);
            ...
            case TK_UNION:
                {
                    int unionTab; /* Cursor number of the temporary table
                        holding result */
                    int op = 0; /* One of the SRT_ operations to apply to self
                        */
                    int priorOp; /* The SRT_ operation to apply to prior
                        selects */
                    Expr *pLimit,  *pOffset; /* Saved values of p->nLimit and p
                        ->nOffset */
                    int addr;
                    priorOp = p->op == TK_ALL ? SRT_Table : SRT_Union;
                    if (eDest == priorOp && pOrderBy == 0 && !p->pLimit && !p
                        ->pOffset)
                    {
                    ...
                    else
                    {
                        ...
                        if (pOrderBy && matchOrderbyToColumn(pParse, p,
                            pOrderBy, unionTab, 1))
                        {
                            rc = 1;
                            goto multi_select_end;
                    ...
                    rc = sqlite3Select(pParse, pPrior, priorOp, unionTab, 0, 0,
                        0, aff);
                    if (rc)
                    {
                        goto multi_select_end;
                    ...
                    if (eDest != priorOp || unionTab != iParm)
                    {
                        ...
                        rc = selectInnerLoop(pParse, p, p->pEList, unionTab, p
                            ->pEList->nExpr, pOrderBy,  - 1, eDest, iParm,
                            iCont, iBreak, 0);
                        if (rc)
                        {
                            rc = 1;
            ...
            case TK_INTERSECT:
                {
                    ...
                    tab1 = pParse->nTab++;
                    tab2 = pParse->nTab++;
                    if (pOrderBy && matchOrderbyToColumn(pParse, p, pOrderBy,
                        tab1, 1))
                    {
                        rc = 1;
                        goto multi_select_end;
                    ...
                    rc = selectInnerLoop(pParse, p, p->pEList, tab1, p->pEList
                        ->nExpr, pOrderBy,  - 1, eDest, iParm, iCont, iBreak, 0)
                        ;
                    if (rc)
                    {
                        rc = 1;
        ...
        if (p->pEList->nExpr != pPrior->pEList->nExpr)
        {
            sqlite3ErrorMsg(pParse, "SELECTs to the left and right of %s"
                " do not have the same number of result columns", selectOpName
                (p->op));
            rc = 1;
            goto multi_select_end;
        }
        ...
        if (pOrderBy || p->usesVirt)
        {
            ...
            pKeyInfo = sqliteMalloc(sizeof(*pKeyInfo) + nCol * 2 * sizeof
                (CollSeq*) + nCol);
            if (!pKeyInfo)
            {
                rc = SQLITE_NOMEM;
            ...
            if (pOrderBy)
            {
                ...
                for (i = 0; i < nOrderByExpr; i++, pOTerm++, apColl++,
                    pSortOrder++)
                {
                ...
                sqlite3VdbeChangeP3(v, addr, (char*)pKeyInfo,
                    P3_KEYINFO_HANDOFF);
                pKeyInfo = 0;
    ...
    static void substExpr(Expr *pExpr, int iTable, ExprList *pEList)
    {
        ...
        if (pExpr->op == TK_COLUMN && pExpr->iTable == iTable)
        {
            ...
            else
            {
                ...
                assert(pExpr->pLeft == 0 && pExpr->pRight == 0 && pExpr->pList
                    == 0);
     ...
     */
    static int flattenSubquery(Select *p,  /* The parent or outer SELECT
        statement */
    ...
    )
    {
         ...
         */
        if (pSubSrc->nSrc > 1 && iFrom > 0 && (pSrc->a[iFrom - 1].jointype
            &JT_OUTER) != 0)
        {
            return 0;
        }
         ...
         */
        if (iFrom > 0 && (pSrc->a[iFrom - 1].jointype &JT_OUTER) != 0 && pSub
            ->pWhere != 0)
        {
            return 0;
        }
        ...
        {
            Expr *pExpr;
            if (pList->a[i].zName == 0 && (pExpr = pList->a[i].pExpr)->span.z
                != 0)
            {
                pList->a[i].zName = sqliteStrNDup((char*)pExpr->span.z, pExpr
                    ->span.n);
            }
        }
        ...
        if (subqueryIsAgg)
        {
            ...
            p->pWhere = pWhere;
            substExpr(p->pHaving, iParent, pSub->pEList);
            p->pHaving = sqlite3ExprAnd(p->pHaving, sqlite3ExprDup(pSub
                ->pHaving));
            assert(p->pGroupBy == 0);
...
static int simpleMinMaxQuery(Parse *pParse, Select *p, int eDest, int iParm)
{
     ...
     ** zero if it is  not.
     */
    if (p->pGroupBy || p->pHaving || p->pWhere)
        return 0;
    pSrc = p->pSrc;
    if (pSrc->nSrc != 1)
        return 0;
    ...
    else
    {
        ...
        for (pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext)
        {
            ...
            if (pIdx->aiColumn[0] == iCol && 0 == sqlite3StrICmp(pIdx
                ->azColl[0], pColl->zName))
            {
                break;
            }
        }
    ...
    else
    {
        ...
        sqlite3VdbeOp3(v, OP_OpenRead, iIdx, pIdx->tnum, (char*)pKey,
            P3_KEYINFO_HANDOFF);
        if (seekOp == OP_Rewind)
        {
 ...
 */
static int processOrderGroupBy(NameContext *pNC,  /* Name context of the SELECT
    statement. */
    ...
    for (i = 0; i < pOrderBy->nExpr; i++)
    {
        ...
        if (sqlite3ExprIsInteger(pE, &iCol))
        {
            if (iCol > 0 && iCol <= pEList->nExpr)
            {
                ...
                pE = pOrderBy->a[i].pExpr = sqlite3ExprDup(pEList->a[iCol -
                    1].pExpr);
            }
            else
            {
 ...
 */
int sqlite3SelectResolve(Parse *pParse,  /* The parser context */
Select *p,  /* The SELECT statement being coded. */
...
)
{
    ...
    memset(&sNC, 0, sizeof(sNC));
    sNC.pParse = pParse;
    if (sqlite3ExprResolveNames(&sNC, p->pLimit) || sqlite3ExprResolveNames
        (&sNC, p->pOffset))
    {
        return SQLITE_ERROR;
    }
     ...
     */
    sNC.pEList = p->pEList;
    if (sqlite3ExprResolveNames(&sNC, p->pWhere) || sqlite3ExprResolveNames
        (&sNC, p->pHaving) || processOrderGroupBy(&sNC, p->pOrderBy, "ORDER")
    ...
    for (pFunc = pAggInfo->aFunc, i = 0; i < pAggInfo->nFunc; i++, pFunc++)
    {
        ...
        if (pFunc->iDistinct >= 0)
        {
            ...
            else
            {
                ...
                sqlite3VdbeOp3(v, OP_OpenVirtual, pFunc->iDistinct, 0, (char*)
                    pKeyInfo, P3_KEYINFO_HANDOFF);
            }
        }
    }
}
...
static void updateAccumulator(Parse *pParse, AggInfo *pAggInfo)
{
    ...
    for (i = 0, pF = pAggInfo->aFunc; i < pAggInfo->nFunc; i++, pF++)
    {
        ...
        sqlite3VdbeOp3(v, OP_AggStep, pF->iMem, nArg, (void*)pF->pFunc,
            P3_FUNCDEF);
        if (addrNext)
        {
 ...
 */
int sqlite3Select(Parse *pParse,  /* The parser context */
Select *p,  /* The SELECT statement being coded. */
int eDest,  /* How to dispose of the results */
...
)
{
     ...
     */
        ...
        for (i = 0; i < pTabList->nSrc; i++)
        {
            ...
            sqlite3Select(pParse, pItem->pSelect, SRT_VirtualTab, pItem
                ->iCursor, p, i, &isAgg, 0);
     ...
     */
    #ifndef SQLITE_OMIT_VIEW
        if (pParent && pParentAgg && flattenSubquery(pParent, parentTab,
            *pParentAgg, isAgg))
        {
            if (isAgg)
                 *pParentAgg = 1;
    ...
    if (pOrderBy)
    {
        ...
        for (i = 0, pTerm = pOrderBy->a; i < pOrderBy->nExpr; i++, pTerm++)
        {
            if (pTerm->zName)
            {
                pTerm->pExpr->pColl = sqlite3LocateCollSeq(pParse, pTerm->zName,
                    - 1);
            }
        }
        if (pParse->nErr)
        ...
        pOrderBy->iECursor = pParse->nTab++;
        p->addrOpenVirt[2] = addrSortIndex = sqlite3VdbeOp3(v, OP_OpenVirtual,
            pOrderBy->iECursor, pOrderBy->nExpr + 2, (char*)pKeyInfo,
            P3_KEYINFO_HANDOFF);
    }
    else
    {
    ...
    if (isDistinct)
    {
        ...
        sqlite3VdbeOp3(v, OP_OpenVirtual, distinct, 0, (char*)pKeyInfo,
            P3_KEYINFO_HANDOFF);
    }
    else
    {
    ...
    else
    {
        ...
        int addrInitializeLoop; /* Start of code that initializes the input
            loop */
        ...
        if (pGroupBy)
        {
            ...
            addrSortingIdx = sqlite3VdbeOp3(v, OP_OpenVirtual,
                sAggInfo.sortingIdx, sAggInfo.nSortingColumn, (char*)pKeyInfo,
                P3_KEYINFO_HANDOFF);
            ...
            rc = selectInnerLoop(pParse, p, p->pEList, 0, 0, pOrderBy, distinct,
                eDest, iParm, addrOutputRow + 1, addrSetAbort, aff);
            if (rc)
            {
                goto select_end;
            }
            ...
            else
            {
                ...
                for (i = 0; i < sAggInfo.nColumn; i++)
                {
                    ...
                    {
                        sqlite3VdbeAddOp(v, OP_Column, pCol->iTable, pCol
                            ->iColumn);
                    }
                    j++;
                }
            ...
            for (j = 0; j < pGroupBy->nExpr; j++)
            {
                ...
                sqlite3VdbeAddOp(v, OP_MemStore, iBMem + j, j < pGroupBy->nExpr
                    - 1);
            }
            for (j = pGroupBy->nExpr - 1; j >= 0; j--)
            {
                ...
                sqlite3VdbeChangeP3(v,  - 1, (void*)pKeyInfo->aColl[j],
                    P3_COLLSEQ);
            }
        ...
        else
        {
            ...
            selectInnerLoop(pParse, p, p->pEList, 0, 0, 0,  - 1, eDest, iParm,
                addrEnd, addrEnd, aff);
        }
