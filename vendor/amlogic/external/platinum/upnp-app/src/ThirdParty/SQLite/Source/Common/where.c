 ...
 */
static WhereTerm *findTerm(WhereClause *pWC,  /* The WHERE clause to be
    searched */
    ...
    for (pTerm = pWC->a, k = pWC->nTerm; k; k--, pTerm++)
    {
            ...
            pTerm->leftColumn == iColumn && (pTerm->eOperator &op) != 0)
        {
            if (iCur >= 0 && pIdx)
            {
                ...
                for (j = 0; j < pIdx->nColumn && pIdx->aiColumn[j] != iColumn;
                    j++) {}
 ...
 **
 */
static void exprAnalyzeAll(SrcList *pTabList,  /* the FROM clause */
...
)
{
     ...
     */
    static int isLikeOrGlob(sqlite3 *db,  /* The database */
    ...
    )
    {
        ...
        if ((pColl->type != SQLITE_COLL_BINARY || noCase) && (pColl->type !=
            SQLITE_COLL_NOCASE || !noCase))
        {
            return 0;
        }
        sqlite3DequoteExpr(pRight);
        z = (char*)pRight->token.z;
        for (cnt = 0; (c = z[cnt]) != 0 && c != wc[0] && c != wc[1] && c !=
            wc[2]; cnt++) {}
 ...
 */
static void exprAnalyze(SrcList *pSrc,  /* the FROM clause */
ExprMaskSet *pMaskSet,  /* table masks */
    ...
    if (allowedOp(pExpr->op) && (pTerm->prereqRight &prereqLeft) == 0)
    {
        ...
        else if (pExpr->op == TK_OR)
        {
            ...
            do
            {
                ...
                ok = iCursor >= 0;
                for (i = sOr.nTerm - 1, pOrTerm = sOr.a; i >= 0 && ok; i--,
                    pOrTerm++)
                {
                    ...
                    if (pOrTerm->leftCursor == iCursor && pOrTerm->leftColumn
                        == iColumn)
                    {
                        pOrTerm->flags |= TERM_OR_OK;
                    }
                    else if ((pOrTerm->flags &TERM_COPIED) != 0 || ((pOrTerm
                        ->flags &TERM_VIRTUAL) != 0 && (sOr.a[pOrTerm
                        ->iParent].flags &TERM_OR_OK) != 0))
                    {
                        pOrTerm->flags &= ~TERM_OR_OK;
                    }
                    else
            ...
            or_not_possible: whereClauseClear(&sOr);
        }
 ...
 */
static int isSortingIndex(Parse *pParse,  /* Parsing context */
...
)
{
    ...
    for (i = j = 0, pTerm = pOrderBy->a; j < nTerm && i < pIdx->nColumn; i++)
    {
        ...
        if (!pColl)
            pColl = db->pDfltColl;
        if (pExpr->iColumn != pIdx->aiColumn[i] || sqlite3StrICmp(pColl->zName,
            pIdx->azColl[i]))
        {
 ...
 */
static int sortableByRowid(int base,  /* Cursor number for table to be sorted */
ExprList *pOrderBy,  /* The ORDER BY clause */
int *pbRev /* Set to 1 if ORDER BY is DESC */
)
{
    ...
    if (pOrderBy->nExpr == 1 && p->op == TK_COLUMN && p->iTable == base && p
        ->iColumn ==  - 1)
    {
 ...
 **
 */
static double bestIndex(Parse *pParse,  /* The parsing context */
WhereClause *pWC,  /* The WHERE clause */
...
)
{
     ...
     */
    if (pProbe == 0 && findTerm(pWC, iCur,  - 1, 0, WO_EQ | WO_IN | WO_LT |
        WO_LE | WO_GT | WO_GE, 0) == 0 && (pOrderBy == 0 || !sortableByRowid
        (iCur, pOrderBy, &rev)))
    {
        *pFlags = 0;
        *ppIndex = 0;
     ...
     */
    pTerm = findTerm(pWC, iCur,  - 1, notReady, WO_LT | WO_LE | WO_GT | WO_GE,
        0);
    if (pTerm)
    {
    ...
    for (; pProbe; pProbe = pProbe->pNext)
    {
        ...
        nEq = i;
        if (pProbe->onError != OE_None && (flags &WHERE_COLUMN_IN) == 0 && nEq
            == pProbe->nColumn)
        {
            flags |= WHERE_UNIQUE;
        }
        ...
        if (nEq < pProbe->nColumn)
        {
            ...
            pTerm = findTerm(pWC, iCur, j, notReady, WO_LT | WO_LE | WO_GT |
                WO_GE, pProbe);
            if (pTerm)
            {
        ...
        if (pOrderBy)
        {
            if ((flags &WHERE_COLUMN_IN) == 0 && isSortingIndex(pParse, pProbe,
                iCur, pOrderBy, nEq, &rev))
            {
                if (flags == 0)
                {
                    flags = WHERE_COLUMN_RANGE;
    ...
    *ppIndex = bestIdx;
    TRACE(("best index is %s, cost=%.9g, flags=%x, nEq=%d\n", bestIdx ? bestIdx
        ->zName: "(none)", lowestCost, bestFlags, bestNEq));
    *pFlags = bestFlags;
    *pnEq = bestNEq;
    return lowestCost;
}
 ...
 **
 */
static void buildIndexProbe(Vdbe *v, int nColumn, int nExtra, int brk, Index
    *pIdx)
{
 ...
 */
static void codeEqualityTerm(Parse *pParse,  /* The parsing context */
...
)
{
    ...
    if (pX->op != TK_IN)
    {
        ...
        else
        {
            ...
            pLevel->nIn++;
            sqliteReallocOrFree((void **) &pLevel->aInLoop, sizeof(pLevel
                ->aInLoop[0]) *3 * pLevel->nIn);
            aIn = pLevel->aInLoop;
            if (aIn)
            {
                aIn += pLevel->nIn *3-3;
 ...
 ** use.
 */
static void codeAllEqualityTerms(Parse *pParse,  /* Parsing context */
 ...
 */
WhereInfo *sqlite3WhereBegin(Parse *pParse,  /* The parser context */
...
)
{
    ...
    for (i = iFrom = 0, pLevel = pWInfo->a; i < pTabList->nSrc; i++, pLevel++)
    {
        ...
        for (j = iFrom, pTabItem = &pTabList->a[j]; j < pTabList->nSrc; j++,
            pTabItem++)
        {
            if (once && ((pTabItem->jointype &(JT_LEFT | JT_CROSS)) != 0 || (j
                > 0 && (pTabItem[ - 1].jointype &(JT_LEFT | JT_CROSS)) != 0)))
            {
                break;
            }
            ...
            cost = bestIndex(pParse, &wc, pTabItem, notReady, (i == 0 &&
                ppOrderBy) ?  *ppOrderBy: 0,  &pIdx, &flags, &nEq);
            if (cost < lowestCost)
            {
                once = 1;
                lowestCost = cost;
    ...
    for (i = 0, pLevel = pWInfo->a; i < pTabList->nSrc; i++, pLevel++)
    {
        ...
        int iIdxCur = pLevel->iIdxCur;
            ...
            if (pParse->explain == 2)
            {
                ...
                sqlite3VdbeOp3(v, OP_Explain, i, pLevel->iFrom, zMsg,
                    P3_DYNAMIC);
            }
        ...
        if (pTab->isTransient || pTab->pSelect)
            continue;
        ...
        if ((pIx = pLevel->pIdx) != 0)
        {
            ...
            sqlite3VdbeOp3(v, OP_OpenRead, iIdxCur, pIx->tnum, (char*)pKey,
                P3_KEYINFO_HANDOFF);
        }
        if ((pLevel->flags &WHERE_IDX_ONLY) != 0)
        {
    ...
    for (i = 0, pLevel = pWInfo->a; i < pTabList->nSrc; i++, pLevel++)
    {
        ...
        else if (pLevel->flags &WHERE_ROWID_RANGE)
        {
            ...
            if (pStart)
            {
                ...
                sqlite3ExprCode(pParse, pX->pRight);
                sqlite3VdbeAddOp(v, OP_ForceInt, pX->op == TK_LE || pX->op ==
                    TK_GT, brk);
    ...
    whereBeginNoMem: whereClauseClear(&wc);
    sqliteFree(pWInfo);
    return 0;
}
...
void sqlite3WhereEnd(WhereInfo *pWInfo)
{
    ...
    for (i = pTabList->nSrc - 1; i >= 0; i--)
    {
        ...
        if (pLevel->nIn)
        {
            int *a;
            int j;
            for (j = pLevel->nIn, a = &pLevel->aInLoop[j *3-3]; j > 0; j--, a
                -= 3)
            {
