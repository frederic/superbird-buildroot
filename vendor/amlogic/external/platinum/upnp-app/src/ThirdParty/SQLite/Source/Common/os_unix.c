...
#include "os.h"
     ...
     */
        ...
        struct lockKey
        {
            ...
            #ifdef SQLITE_UNIX_THREADS
                pthread_t tid; /* Thread ID or zero if threads can override
                    each other */
            #endif
        };
        ...
        {
            SQLITE_HASH_BINARY, 0, 0, 0, sqlite3ThreadSafeMalloc,
                sqlite3ThreadSafeFree, 0, 0
        };
        static Hash openHash =
        {
            ...
            struct threadTestData
            {
                ...
                static int lockTrace(int fd, int op, struct flock *p)
                {
                    ...
                    sqlite3DebugPrintf("fcntl %d %d %s %s %d %d %d %d\n",
                        threadid, fd, zOpName, zType, (int)p->l_start, (int)p
                        ->l_len, (int)p->l_pid, s);
                    if (s && op == F_SETLK && (p->l_type == F_RDLCK || p
                        ->l_type == F_WRLCK))
                    {
                        struct flock l2;
                        l2 =  *p;
                        ...
                        sqlite3DebugPrintf(
                            ...
                            l2.l_start, (int)l2.l_len, (int)l2.l_pid);
                    }
                    errno = savedErrno;
                    return s;
                }
            ...
            static void testThreadLockingBehavior(int fd_orig)
            {
                ...
                pthread_join(t[1], 0);
                close(fd);
                threadsOverrideEachOthersLocks = d[0].result == 0 &&
                    d[1].result == 0;
            }
         ...
         */
        static int findLockInfo(int fd,  /* The file descriptor used in the key
            */
        ...
        )
        {
            ...
            pLock = (struct lockInfo*)sqlite3HashFind(&lockHash, &key1, sizeof
                (key1));
            if (pLock == 0)
            {
                ...
                pLock->cnt = 0;
                pLock->locktype = 0;
                pOld = sqlite3HashInsert(&lockHash, &pLock->key, sizeof(key1),
                    pLock);
                if (pOld != 0)
                {
                    assert(pOld == pLock);
            ...
            *ppLock = pLock;
            if (ppOpen != 0)
            {
                pOpen = (struct openCnt*)sqlite3HashFind(&openHash, &key2,
                    sizeof(key2));
                if (pOpen == 0)
                {
                    struct openCnt *pOld;
                    ...
                    pOpen->nPending = 0;
                    pOpen->aPending = 0;
                    pOld = sqlite3HashInsert(&openHash, &pOpen->key, sizeof
                        (key2), pOpen);
                    if (pOld != 0)
                    {
                        assert(pOld == pOpen);
         ...
         */
            ...
            static int transferOwnership(unixFile *pFile)
            {
                ...
                TRACE5("LOCK    %d is now %s(%s,%d)\n", pFile->h, locktypeName
                    (pFile->locktype), locktypeName(pFile->pLock->locktype),
                    pFile->pLock->cnt);
                return rc;
            }
        #else
         ...
         */
        int sqlite3UnixOpenReadWrite(const char *zFilename, OsFile **pId, int
            *pReadonly)
        {
            int rc;
            unixFile f;
            CRASH_TEST_OVERRIDE(sqlite3CrashOpenReadWrite, zFilename, pId,
                pReadonly);
            assert(0 ==  *pId);
         ...
         */
        int sqlite3UnixOpenExclusive(const char *zFilename, OsFile **pId, int
            delFlag)
        {
            int rc;
            unixFile f;
            CRASH_TEST_OVERRIDE(sqlite3CrashOpenExclusive, zFilename, pId,
                delFlag);
            assert(0 ==  *pId);
            ...
            f.h = open(zFilename, O_RDWR | O_CREAT | O_EXCL | O_NOFOLLOW |
                O_LARGEFILE | O_BINARY, SQLITE_DEFAULT_FILE_PERMISSIONS);
            if (f.h < 0)
            {
                return SQLITE_CANTOPEN;
            }
        ...
        static int unixWrite(OsFile *id, const void *pBuf, int amt)
        {
            ...
            TIMER_START;
            while (amt > 0 && (wrote = seekAndWrite((unixFile*)id, pBuf, amt))
                > 0)
            {
                amt -= wrote;
        ...
        static int unixSync(OsFile *id, int dataOnly)
        {
            ...
            if (pFile->dirfd >= 0)
            {
                TRACE4("DIRSYNC %-3d (have_fullfsync=%d fullsync=%d)\n", pFile
                    ->dirfd, HAVE_FULLFSYNC, pFile->fullSync);
                #ifndef SQLITE_DISABLE_DIRSYNC
                    /* The directory sync is only attempted if full_fsync is
                     ** turned off or unavailable.  If a full_fsync occurred above,
                     ** then the directory sync is superfluous.
                     */
                    if ((!HAVE_FULLFSYNC || !pFile->fullSync) && full_fsync
                        (pFile->dirfd, 0, 0))
                    {
                        /*
                ...
                #endif
                close(pFile->dirfd); /* Only need to sync once, so close the
                    directory */
        ...
        static int unixTruncate(OsFile *id, i64 nByte)
        {
            assert(id);
            SimulateIOError(SQLITE_IOERR);
            return ftruncate(((unixFile*)id)->h, nByte) == 0 ? SQLITE_OK :
                SQLITE_IOERR;
        }
        ...
        static int unixCheckReservedLock(OsFile *id)
        {
            ...
            unixFile *pFile = (unixFile*)id;
            assert(pFile);
            sqlite3OsEnterMutex(); /* Because pFile->pLock is shared across
                threads */
        ...
        static int unixLock(OsFile *id, int locktype)
        {
             ...
             */
            if ((pFile->locktype != pLock->locktype && (pLock->locktype >=
                PENDING_LOCK || locktype > SHARED_LOCK)))
            {
                rc = SQLITE_BUSY;
                goto end_lock;
            }
             ...
             ** return SQLITE_OK.
             */
            if (locktype == SHARED_LOCK && (pLock->locktype == SHARED_LOCK ||
             ...
             ** be released.
             */
            if (locktype == SHARED_LOCK || (locktype == EXCLUSIVE_LOCK && pFile
            ...
            end_lock: sqlite3OsLeaveMutex();
            TRACE4("LOCK    %d %s %s\n", pFile->h, locktypeName(locktype), rc
                == SQLITE_OK ? "ok" : "failed");
            return rc;
        }
        ...
        static int unixClose(OsFile **pId)
        {
            ...
            if (id->pOpen->nLock)
            {
                ...
                aNew = realloc(pOpen->aPending, (pOpen->nPending + 1) *sizeof
                    (int));
                if (aNew == 0)
                {
        ...
        {
            unixClose, unixOpenDirectory, unixRead, unixWrite, unixSeek,
                unixTruncate, unixSync, unixSetFullSync, unixFileHandle,
                unixFileSize, unixLock, unixUnlock, unixLockState,
    ...
    static int inMutex = 0;
        ...
        static pthread_mutex_t mutexMain = PTHREAD_MUTEX_INITIALIZER; /* The
            mutex */
        static pthread_mutex_t mutexAux = PTHREAD_MUTEX_INITIALIZER; /* Aux
            mutex */
    #endif
    ...
    int sqlite3UnixInMutex(int thisThrd)
    {
            ...
            int rc;
            pthread_mutex_lock(&mutexAux);
            rc = inMutex > 0 && (thisThrd == 0 || pthread_equal(mutexOwner,
                pthread_self()));
            pthread_mutex_unlock(&mutexAux);
            return rc;
        #else
     ...
     */
        ...
        #ifdef SQLITE_UNIX_THREADS
            static pthread_mutex_t tsd_counter_mutex =
    ...
    ThreadData *sqlite3UnixThreadSpecificData(int allocateFlag)
    {
        ...
        {
            0
        }; /* Initializer to silence warnings
    ...
    int sqlite3UnixCurrentTime(double *prNow)
    {
            ...
            *prNow = 2440587.5 + sNow.tv_sec / 86400.0 + sNow.tv_usec /
                86400000000.0;
        #endif
        #ifdef SQLITE_TEST
