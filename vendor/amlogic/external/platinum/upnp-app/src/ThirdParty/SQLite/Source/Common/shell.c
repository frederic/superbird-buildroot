...
static void shellstaticFunc(sqlite3_context *context, int argc, sqlite3_value
    **argv)
{
    assert(0 == argc);
    assert(zShellStatic);
...
{
    "line", "column", "list", "semi", "html", "insert", "tcl", "csv",
};
        ...
        case MODE_Column:
            {
                if (p->cnt++ == 0)
                {
                    for (i = 0; i < nArg; i++)
                    {
                        ...
                        if (w <= 0)
                        {
                            ...
                            if (w < 10)
                                w = 10;
                            n = strlen(azArg && azArg[i] ? azArg[i]: p
                                ->nullvalue);
                            if (w < n)
                                w = n;
                        }
                        ...
                        if (p->showHeader)
                        {
                            fprintf(p->out, "%-*.*s%s", w, w, azCol[i], i ==
                                nArg - 1 ? "\n" : "  ");
                        }
                    }
                    if (p->showHeader)
                ...
                for (i = 0; i < nArg; i++)
                {
                    ...
                    fprintf(p->out, "%-*.*s%s", w, w, azArg[i] ? azArg[i]: p
                        ->nullvalue, i == nArg - 1 ? "\n" : "  ");
                }
                break;
            }
        ...
        case MODE_Html:
            {
                ...
                {
                    fprintf(p->out, "<TD>");
                    output_html_string(p->out, azArg[i] ? azArg[i]: p
                        ->nullvalue);
                    fprintf(p->out, "</TD>\n");
                }
                fprintf(p->out, "</TR>\n");
...
static int do_meta_command(char *zLine, struct callback_data *p)
{
    ...
    else if (c == 'h' && (strncmp(azArg[0], "header", n) == 0 || strncmp
        (azArg[0], "headers", n) == 0) && nArg > 1)
    {
        int j;
        char *z = azArg[1];
        int val = atoi(azArg[1]);
    ...
    else if (c == 'i' && strncmp(azArg[0], "indices", n) == 0 && nArg > 1)
    {
        ...
        sqlite3_exec(p->db, "SELECT name FROM sqlite_master "
            "WHERE type='index' AND tbl_name LIKE shellstatic() ""UNION ALL "
            "SELECT name FROM sqlite_temp_master "
            "WHERE type='index' AND tbl_name LIKE shellstatic() ""ORDER BY 1",
    ...
    else if (c == 'm' && strncmp(azArg[0], "mode", n) == 0 && nArg >= 2)
    {
        ...
        if (strncmp(azArg[1], "line", n2) == 0 || strncmp(azArg[1], "lines", n2)
            == 0)
        {
            p->mode = MODE_Line;
        }
        else if (strncmp(azArg[1], "column", n2) == 0 || strncmp(azArg[1],
            "columns", n2) == 0)
        {
            p->mode = MODE_Column;
        }
    ...
    {
        sprintf(p->nullvalue, "%.*s", (int)ArraySize(p->nullvalue) - 1,
            azArg[1]);
    }
    ...
    else if (c == 'p' && strncmp(azArg[0], "prompt", n) == 0 && (nArg == 2 ||
        nArg == 3))
    {
        if (nArg >= 2)
        {
        ...
        if (nArg >= 3)
        {
            strncpy(continuePrompt, azArg[2], (int)ArraySize(continuePrompt) -
                1);
        }
    }
    ...
    else if (c == 's' && strncmp(azArg[0], "schema", n) == 0)
    {
        ...
        if (nArg > 1)
        {
            ...
            {
                zShellStatic = azArg[1];
                sqlite3_exec(p->db, "SELECT sql FROM "
                    "  (SELECT * FROM sqlite_master UNION ALL"
                    "   SELECT * FROM sqlite_temp_master) "
                    "WHERE tbl_name LIKE shellstatic() AND type!='meta' AND sql NOTNULL ""ORDER BY substr(type,2,1), name", callback, &data, &zErrMsg);
                zShellStatic = 0;
            }
        }
        else
        {
            sqlite3_exec(p->db, "SELECT sql FROM "
                "  (SELECT * FROM sqlite_master UNION ALL"
                "   SELECT * FROM sqlite_temp_master) "
                "WHERE type!='meta' AND sql NOTNULL AND name NOT LIKE 'sqlite_%'""ORDER BY substr(type,2,1), name", callback, &data, &zErrMsg);
        }
        if (zErrMsg)
        {
            fprintf(stderr, "Error: %s\n", zErrMsg);
    ...
    {
        sprintf(p->separator, "%.*s", (int)ArraySize(p->separator) - 1,
            azArg[1]);
    }
    ...
    else if (c == 't' && n > 1 && strncmp(azArg[0], "tables", n) == 0)
    {
        ...
        if (nArg == 1)
        {
            rc = sqlite3_get_table(p->db, "SELECT name FROM sqlite_master "
                ...
                "WHERE type IN ('table','view') ""ORDER BY 1",  &azResult,
                &nRow, 0, &zErrMsg);
        }
        else
        {
            zShellStatic = azArg[1];
            rc = sqlite3_get_table(p->db, "SELECT name FROM sqlite_master "
                "WHERE type IN ('table','view') AND name LIKE '%'||shellstatic()||'%' ""UNION ALL ""SELECT name FROM sqlite_temp_master ""WHERE type IN ('table','view') AND name LIKE '%'||shellstatic()||'%' ""ORDER BY 1",  &azResult, &nRow, 0, &zErrMsg);
            zShellStatic = 0;
        }
        if (zErrMsg)
        {
    ...
    else if (c == 't' && n > 1 && strncmp(azArg[0], "timeout", n) == 0 && nArg
        >= 2)
    {
        open_db(p);
...
static int _is_command_terminator(const char *zLine)
{
    ...
    if (zLine[0] == '/' && _all_whitespace(&zLine[1]))
        return 1;
     /* Oracle */
    if (tolower(zLine[0]) == 'g' && tolower(zLine[1]) == 'o' && _all_whitespace
        (&zLine[2]))
    {
        return 1; /* SQL Server */
    }
 ...
 */
static void process_sqliterc(struct callback_data *p,  /* Configuration data */
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc - 1; i++)
    {
        if (argv[i][0] != '-')
            break;
        if (strcmp(argv[i], "-separator") == 0 || strcmp(argv[i], "-nullvalue")
            == 0)
        {
            i++;
        }
