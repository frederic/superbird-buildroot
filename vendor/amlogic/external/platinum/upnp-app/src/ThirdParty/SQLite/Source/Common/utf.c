...
int sqlite3utf8CharLen(const char *z, int nByte)
{
     ...
     */
    void sqlite3utf16Substr(sqlite3_context *context, int argc, sqlite3_value
        **argv)
    {
        int y, z;
        ...
        else
        {
            ...
            if (SQLITE_UTF16BE == SQLITE_UTF16NATIVE)
            {
                for (i = y; i < 0 && zStart > zStr; i++)
                    RSKIP_UTF16BE(zStart);
            }
            else
            {
                for (i = y; i < 0 && zStart > zStr; i++)
                    RSKIP_UTF16LE(zStart);
            }
            for (; i < 0; i++)
                z -= 1;
        }
