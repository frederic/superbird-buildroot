...
static int selectReadsTable(Select *p, Schema *pSchema, int iTab)
{
    ...
    for (i = 0, pItem = p->pSrc->a; i < p->pSrc->nSrc; i++, pItem++)
    {
        if (pItem->pSelect)
        {
            if (selectReadsTable(pItem->pSelect, pSchema, iTab))
                return 1;
        }
        else
        {
            if (pItem->pTab->pSchema == pSchema && pItem->pTab->tnum == iTab)
                return 1;
        }
    }
    return 0;
 ...
 **         cleanup
 */
void sqlite3Insert(Parse *pParse,  /* Parser context */
...
)
{
    ...
    if (triggers_exist)
    {
        ...
        if (pTab->autoInc)
        {
            ...
            sqlite3OpenTable(pParse, iCur, iDb, pDb->pSchema->pSeqTab,
                OP_OpenRead);
    ...
    if (pSelect)
    {
        ...
        rc = sqlite3Select(pParse, pSelect, SRT_Subroutine, iInsertBlock, 0, 0,
            0, 0);
         ...
         */
        if (triggers_exist || selectReadsTable(pSelect, pTab->pSchema, pTab
            ->tnum))
        {
            useTempTable = 1;
        }
    ...
    if (pColumn == 0 && nColumn != pTab->nCol)
    {
        sqlite3ErrorMsg(pParse,
            "table %S has %d columns but %d values were supplied", pTabList, 0,
            pTab->nCol, nColumn);
        goto insert_cleanup;
    }
    if (pColumn != 0 && nColumn != pColumn->nId)
    {
        sqlite3ErrorMsg(pParse, "%d values for %d columns", nColumn, pColumn
            ->nId);
        goto insert_cleanup;
    }
    ...
    if (pColumn)
    {
        ...
        for (i = 0; i < pColumn->nId; i++)
        {
            ...
            {
                if (sqlite3StrICmp(pColumn->a[i].zName, pTab->aCol[j].zName) ==
                    0)
                {
                    pColumn->a[i].idx = j;
                    if (j == pTab->iPKey)
    ...
    if (!isView)
    {
        ...
        sqlite3GenerateConstraintChecks(pParse, pTab, base, 0, keyColumn >= 0,
            0, onError, endOfLoop);
        sqlite3CompleteInsertion(pParse, pTab, base, 0, 0, 0, (triggers_exist
            &TRIGGER_AFTER) != 0 ? newIdx :  - 1);
    }
     ...
     */
    if (db->flags &SQLITE_CountRows && pParse->nested == 0 && !pParse
        ->trigStack)
    {
    ...
    insert_cleanup: sqlite3SrcListDelete(pTabList);
 ...
 */
void sqlite3GenerateConstraintChecks(Parse *pParse,  /* The parser context */
...
)
{
    ...
    for (i = 0; i < nCol; i++)
    {
        ...
        assert(onError == OE_Rollback || onError == OE_Abort || onError ==
            OE_Fail || onError == OE_Ignore || onError == OE_Replace);
        switch (onError)
        {
            case OE_Rollback:
            case OE_Abort:
            case OE_Fail:
                {
                    ...
                    sqlite3SetString(&zMsg, pTab->zName, ".", pTab
                        ->aCol[i].zName, " may not be NULL", (char*)0);
    ...
    for (iCur = 0, pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext, iCur++)
    {
        ...
        switch (onError)
        {
            ...
            case OE_Fail:
                {
                    ...
                    n1 = strlen(zErrMsg);
                    for (j = 0; j < pIdx->nColumn && n1 < sizeof(zErrMsg) - 30;
                        j++)
                    {
                    ...
                    strcpy(&zErrMsg[n1], pIdx->nColumn > 1 ? " are not unique"
                        : " is not unique");
                    sqlite3VdbeOp3(v, OP_Halt, SQLITE_CONSTRAINT, onError,
                        zErrMsg, 0);
                    break;
            ...
            case OE_Ignore:
                {
                    assert(seenReplace == 0);
                    sqlite3VdbeAddOp(v, OP_Pop, nCol + extra + 3+hasTwoRowids,
                        0);
                    sqlite3VdbeAddOp(v, OP_Goto, 0, ignoreDest);
                    break;
                }
 ...
 */
void sqlite3CompleteInsertion(Parse *pParse,  /* The parser context */
Table *pTab,  /* the table into which we are inserting */
int base,  /* Index of a read/write cursor pointing at pTab */
char *aIdxUsed,  /* Which indices are used.  NULL means all are used */
int rowidChng,  /* True if the record number will change */
 ...
 */
void sqlite3OpenTableAndIndices(Parse *pParse,  /* Parsing context */
...
)
{
    ...
    for (i = 1, pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext, i++)
    {
        ...
        sqlite3VdbeOp3(v, op, i + base, pIdx->tnum, (char*)pKey,
            P3_KEYINFO_HANDOFF);
    }
