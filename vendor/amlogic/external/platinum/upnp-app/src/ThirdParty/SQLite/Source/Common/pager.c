 ...
 */
    ...
    struct PgHdr
    {
        ...
        PgHdr *pNextStmt,  *pPrevStmt; /* List of pages in the statement
            journal */
     ...
     */
        ...
        #define CODEC1(P,D,N,X) if ( P->xCodec!=0 ) { P->xCodec(P->pCodecArg,D,N,
            X); }
        #define CODEC2(P,D,N,X) ((char*)(P->xCodec!=0?P->xCodec(P->pCodecArg,D,
            N,X):D))
    #else
    ...
    struct Pager
    {
        ...
        #endif
        void(*xDestructor)(void *, int); /* Call this routine when freeing
            pages */
        void(*xReiniter)(void *, int); /* Call this routine when reloading
            pages */
     ...
     */
        ...
        {
            static int cnt = 0;
            if (!pager3_refinfo_enable)
                return ;
            sqlite3DebugPrintf("REFCNT: %4d addr=%p nRef=%d\n", p->pgno,
                PGHDR_TO_DATA(p), p->nRef);
    ...
    static int pager_error(Pager *pPager, int rc)
    {
        ...
        if (rc == SQLITE_FULL || rc == SQLITE_IOERR || rc == SQLITE_CORRUPT ||
            rc == SQLITE_PROTOCOL)
        {
            pPager->errCode = rc;
        }
    ...
    static int seekJournalHdr(Pager *pPager)
    {
        ...
        if (c)
        {
            offset = ((c - 1) / JOURNAL_HDR_SZ(pPager) + 1) *JOURNAL_HDR_SZ
                (pPager);
        }
    ...
    static int writeJournalHdr(Pager *pPager)
    {
        ...
        put32bits(&zHeader[sizeof(aJournalMagic)], pPager->noSync ? 0xffffffff
            : 0);
     ...
     */
    static int readJournalHdr(Pager *pPager, i64 journalSize, u32 *pNRec, u32
        *pDbSize)
    {
        int rc;
        unsigned char aMagic[8]; /* A buffer to hold the magic header */
    ...
    static int pager_playback_one_page(Pager *pPager, OsFile *jfd, int useCksum)
    {
        ...
        assert(pPager->state == PAGER_RESERVED || pPager->state >=
            PAGER_EXCLUSIVE);
    ...
    static int pager_delmaster(const char *zMaster)
    {
        ...
        delmaster_out: if (zMasterJournal)
        {
    ...
    static int pager_reload_cache(Pager *pPager)
    {
        ...
        for (pPg = pPager->pAll; pPg; pPg = pPg->pNextAll)
        {
            ...
            {
                rc = sqlite3OsSeek(pPager->fd, pPager->pageSize *(i64)(pPg
                    ->pgno - 1));
                if (rc == SQLITE_OK)
                {
            ...
            if (pPg->nRef == 0 || memcmp(zBuf, PGHDR_TO_DATA(pPg), pPager
                ->pageSize))
            {
    ...
    static int pager_stmt_playback(Pager *pPager)
    {
        ...
        pPager->journalOff = szJ;
        end_stmt_playback: if (rc == SQLITE_OK)
        {
            pPager->journalOff = szJ;
     ...
     */
        ...
        void sqlite3pager_set_safety_level(Pager *pPager, int level, int
            full_fsync)
        {
     ...
     ** in-memory database.
     */
    int sqlite3pager_open(Pager **ppPager,  /* Return the Pager structure here
        */
    ...
    )
    {
        ...
        int nameLen; /* Compiler is wrong. This is always initialized before
            use */
        OsFile *fd;
        int rc = SQLITE_OK;
        int i;
     ...
     */
    void sqlite3pager_read_fileheader(Pager *pPager, int N, unsigned char
        *pDest)
    {
        memset(pDest, 0, N);
        if (MEMDB == 0)
    ...
    static int pager_wait_on_lock(Pager *pPager, int locktype)
    {
        ...
        else
        {
            ...
            while (rc == SQLITE_BUSY && sqlite3InvokeBusyHandler(pPager
                ->pBusyHandler));
            if (rc == SQLITE_OK)
            {
    ...
    int sqlite3pager_close(Pager *pPager)
    {
        ...
        TRACE2("CLOSE %d\n", PAGERID(pPager));
        assert(pPager->errCode || (pPager->journalOpen == 0 && pPager->stmtOpen
            == 0));
        if (pPager->journalOpen)
        {
            sqlite3OsClose(&pPager->jfd);
         ...
         */
            ...
            {
                Pager *pTmp;
                for (pTmp = pTsd->pPager; pTmp->pNext != pPager; pTmp = pTmp
                    ->pNext) {}
        ...
        if (pPager->needSync)
        {
            if (!pPager->tempFile)
            {
                ...
                #endif
                {
                    ...
                    rc = sqlite3OsSeek(pPager->jfd, pPager->journalHdr + sizeof
                        (aJournalMagic));
                    if (rc)
                        return rc;
    ...
    static int pager_write_pagelist(PgHdr *pList)
    {
        ...
        {
            assert(pList->dirty);
            rc = sqlite3OsSeek(pPager->fd, (pList->pgno - 1)*(i64)pPager
                ->pageSize);
            if (rc)
                return rc;
            ...
            {
                char *pData = CODEC2(pPager, PGHDR_TO_DATA(pList), pList->pgno,
                    6);
     ...
     */
        ...
        int sqlite3pager_release_memory(int nReq)
        {
            ...
            for (i = 0; i <= 1; i++)
            {
                ...
                for (p = pTsdro->pPager; p && (nReq < 0 || nReleased < nReq); p
                    = p->pNext)
                {
                    PgHdr *pPg;
                    int rc;
                    ...
                    while (SQLITE_OK == (rc = pager_recycle(p, i, &pPg)) && pPg)
                    {
                        ...
                        {
                            for (pTmp = p->pAll; pTmp->pNextAll != pPg; pTmp =
                                pTmp->pNextAll) {}
    ...
    int sqlite3pager_get(Pager *pPager, Pgno pgno, void **ppPage)
    {
        ...
        *ppPage = 0;
        ...
        if (pPg == 0)
        {
            ...
            {
                /* Create a new page */
                pPg = sqliteMallocRaw(sizeof(*pPg) + pPager->pageSize + sizeof
                    (u32) + pPager->nExtra + MEMDB * sizeof(PgHistory));
                if (pPg == 0)
                {
                    return SQLITE_NOMEM;
                }
            ...
            if (pPager->aInJournal && (int)pgno <= pPager->origDbSize)
            {
                ...
                assert(pPager->journalOpen);
                pPg->inJournal = (pPager->aInJournal[pgno / 8] &(1 << (pgno &7))
                    ) != 0;
                pPg->needSync = 0;
            }
            else
            ...
            if (pPager->aInStmt && (int)pgno <= pPager->stmtSize && (pPager
                ->aInStmt[pgno / 8] &(1 << (pgno &7))) != 0)
            {
            ...
            else
            {
                ...
                if (rc == SQLITE_OK)
                {
                    rc = sqlite3OsRead(pPager->fd, PGHDR_TO_DATA(pPg), pPager
                        ->pageSize);
                }
    ...
    static int pager_open_journal(Pager *pPager)
    {
        ...
        rc = sqlite3OsOpenExclusive(pPager->zJournal, &pPager->jfd, pPager
            ->tempFile);
        pPager->journalOff = 0;
        pPager->setMaster = 0;
        pPager->journalHdr = 0;
        ...
        return rc;
        failed_to_open_journal: sqliteFree(pPager->aInJournal);
        pPager->aInJournal = 0;
        if (rc == SQLITE_NOMEM)
        {
    ...
    int sqlite3pager_write(void *pData)
    {
        ...
        else
        {
            ...
            if (!pPg->inJournal && (pPager->useJournal || MEMDB))
            {
                if ((int)pPg->pgno <= pPager->origDbSize)
                {
                    ...
                    if (MEMDB)
                    {
                        ...
                        if (pHist->pOrig)
                        {
                            memcpy(pHist->pOrig, PGHDR_TO_DATA(pPg), pPager
                                ->pageSize);
                        }
                    }
                    else
                    {
                        ...
                        *(u32*)pEnd = saved;
                        ...
                        if (pPager->stmtInUse)
                        {
                            pPager->aInStmt[pPg->pgno / 8] |= 1 << (pPg->pgno
                                &7);
                            page_add_to_stmt_list(pPg);
                        }
                    }
             ...
             */
            if (pPager->stmtInUse && !pPg->inStmt && (int)pPg->pgno <= pPager
                ->stmtSize)
            {
                ...
                if (MEMDB)
                {
                    ...
                    if (pHist->pStmt)
                    {
                        memcpy(pHist->pStmt, PGHDR_TO_DATA(pPg), pPager
                            ->pageSize);
                    }
    ...
    void sqlite3pager_dont_write(Pager *pPager, Pgno pgno)
    {
        ...
        if (MEMDB)
            return ;
        pPg = pager_lookup(pPager, pgno);
        assert(pPg != 0); /* We never call _dont_write unless the page is in
            mem */
        pPg->alwaysRollback = 1;
        if (pPg->dirty && !pPager->stmtInUse)
        {
            if (pPager->dbSize == (int)pPg->pgno && pPager->origDbSize < pPager
                ->dbSize)
            {
    ...
    void sqlite3pager_dont_rollback(void *pData)
    {
        PgHdr *pPg = DATA_TO_PGHDR(pData);
        Pager *pPager = pPg->pPager;
        if (pPager->state != PAGER_EXCLUSIVE || pPager->journalOpen == 0)
            return ;
        if (pPg->alwaysRollback || pPager->alwaysRollback || MEMDB)
            return ;
    ...
    int sqlite3pager_stmt_begin(Pager *pPager)
    {
        ...
        pPager->stmtInUse = 1;
        return SQLITE_OK;
        stmt_begin_failed: if (pPager->aInStmt)
        {
        ...
        if (pPager->stmtInUse)
        {
            ...
            if (MEMDB)
            {
                ...
                for (pPg = pPager->pStmt; pPg; pPg = pPg->pNextStmt)
                {
                    ...
                    if (pHist->pStmt)
                    {
                        memcpy(PGHDR_TO_DATA(pPg), pHist->pStmt, pPager
                            ->pageSize);
                        sqliteFree(pHist->pStmt);
                        pHist->pStmt = 0;
                    }
     ...
     */
    void sqlite3pager_set_codec(Pager *pPager, void *(*xCodec)(void *, void *,
        Pgno, int), void *pCodecArg)
    {
    ...
    int sqlite3pager_sync(Pager *pPager, const char *zMaster, Pgno nTrunc)
    {
        ...
        int sqlite3pager_movepage(Pager *pPager, void *pData, Pgno pgno)
        {
            ...
            TRACE5("MOVE %d page %d (needSync=%d) moves to %d\n", PAGERID
                (pPager), pPg->pgno, pPg->needSync, pgno);
            if (pPg->needSync)
            {
