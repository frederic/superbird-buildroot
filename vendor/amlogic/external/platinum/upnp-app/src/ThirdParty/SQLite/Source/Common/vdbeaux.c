...
int sqlite3VdbeMakeLabel(Vdbe *p)
{
    ...
    if (i >= p->nLabelAlloc)
    {
        ...
        sqliteReallocOrFree((void **) &p->aLabel, p->nLabelAlloc *sizeof(p
            ->aLabel[0]));
    }
    if (p->aLabel)
    {
...
static int opcodeNoPush(u8 op)
{
    ...
    {
        NOPUSH_MASK_0 + (((unsigned)NOPUSH_MASK_1) << 16), NOPUSH_MASK_2 + ((
            (unsigned)NOPUSH_MASK_3) << 16), NOPUSH_MASK_4 + (((unsigned)
            NOPUSH_MASK_5) << 16), NOPUSH_MASK_6 + (((unsigned)NOPUSH_MASK_7)
            << 16), NOPUSH_MASK_8 + (((unsigned)NOPUSH_MASK_9) << 16)
    };
    assert(op < 32 *5);
...
void sqlite3VdbeChangeP3(Vdbe *p, int addr, const char *zP3, int n)
{
    ...
    else if (n == P3_KEYINFO)
    {
        ...
        if (pKeyInfo)
        {
            ...
            if (aSortOrder)
            {
                pKeyInfo->aSortOrder = (unsigned char*) &pKeyInfo
                    ->aColl[nField];
...
VdbeOp *sqlite3VdbeGetOp(Vdbe *p, int addr)
{
    ...
    void sqlite3VdbePrintOp(FILE *pOut, int pc, Op *pOp)
    {
        ...
        fprintf(pOut, zFormat1, pc, sqlite3OpcodeNames[pOp->opcode], pOp->p1,
            pOp->p2, zP3);
        fflush(pOut);
    }
#endif
...
static void releaseMemArray(Mem *p, int N)
{
     ...
     */
    int sqlite3VdbeList(Vdbe *p /* The VDBE */
    )
    {
        sqlite3 *db = p->db;
        int i;
 ...
 ** VDBE_MAGIC_RUN.
 */
void sqlite3VdbeMakeReady(Vdbe *p,  /* The VDBE */
...
)
{
    ...
    if (p->aStack == 0)
    {
        ...
        p->aStack = sqliteMalloc(nStack *sizeof(p->aStack[0]) /* aStack */
    ...
    #ifdef SQLITE_DEBUG
        if ((p->db->flags &SQLITE_VdbeListing) != 0 || sqlite3OsFileExists(
            "vdbe_explain"))
        {
            int i;
...
int sqlite3VdbeSetColName(Vdbe *p, int idx, int var, const char *zName, int N)
{
    ...
    {
        rc = sqlite3VdbeMemSetStr(pColName, zName,  - 1, SQLITE_UTF8,
            SQLITE_STATIC);
    }
    else
    {
...
static int vdbeCommit(sqlite3 *db)
{
     ...
     */
        ...
        else
        {
            ...
            if (rc != SQLITE_OK || (needSync && (rc = sqlite3OsSync(master, 0))
                != SQLITE_OK))
            {
                sqlite3OsClose(&master);
                sqlite3OsDelete(zMaster);
...
void sqlite3AbortOtherActiveVdbes(sqlite3 *db, Vdbe *pExcept)
{
    ...
    for (pOther = db->pVdbe; pOther; pOther = pOther->pNext)
    {
        if (pOther == pExcept)
            continue;
        if (pOther->magic != VDBE_MAGIC_RUN || pOther->pc < 0)
            continue;
...
int sqlite3VdbeHalt(Vdbe *p)
{
    ...
    if (p->pc >= 0)
    {
        ...
        isSpecialError = ((p->rc == SQLITE_NOMEM || p->rc == SQLITE_IOERR) ? 1
            : 0);
        if (isSpecialError)
        {
         ...
         ** error value.
         */
        assert(!xFunc || xFunc == sqlite3BtreeCommitStmt || xFunc ==
    ...
    assert(p->pTos <  &p->aStack[p->pc < 0 ? 0 : p->pc] || !p->aStack);
    #ifdef VDBE_PROFILE
        {
            ...
            if (out)
            {
                ...
                for (i = 0; i < p->nOp; i++)
                {
                    fprintf(out, "%6d %10lld %8lld ", p->aOp[i].cnt, p
                        ->aOp[i].cycles, p->aOp[i].cnt > 0 ? p->aOp[i].cycles /
                        p->aOp[i].cnt: 0);
 ...
 */
int sqlite3VdbeSerialGet(const unsigned char *buf,  /* Buffer to deserialize
    from */
...
)
{
    switch (serial_type)
    {
        ...
        case 3:
            ...
            {
                pMem->i = (((signed char)buf[0]) << 16) | (buf[1] << 8) |
                    buf[2];
                pMem->flags = MEM_Int;
                return 3;
            }
        case 4:
            ...
            {
                pMem->i = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) |
                    buf[3];
                pMem->flags = MEM_Int;
                return 4;
            }
        case 5:
             /* 6-byte signed integer */
            {
                ...
                u32 y = (buf[2] << 24) | (buf[3] << 16) | (buf[4] << 8) |
                    buf[5];
                x = (x << 32) | y;
                pMem->i = *(i64*) &x;
                pMem->flags = MEM_Int;
 ...
 */
int sqlite3VdbeRecordCompare(void *userData, int nKey1, const void *pKey1, int
    nKey2, const void *pKey2)
{
    ...
    while (idx1 < szHdr1 && idx2 < szHdr2)
    {
        ...
        if (d2 >= nKey2 && sqlite3VdbeSerialTypeLen(serial_type2) > 0)
            break;
    ...
    else if (pKeyInfo->aSortOrder && i < pKeyInfo->nField && pKeyInfo
        ->aSortOrder[i])
    {
        rc =  - rc;
    }
 ...
 ** is ignored as well.
 */
int sqlite3VdbeIdxKeyCompare(Cursor *pC,  /* The cursor to compare against */
...
)
{
    ...
    *res = sqlite3VdbeRecordCompare(pC->pKeyInfo, m.n - lenRowid, m.z, nKey,
        pKey);
    sqlite3VdbeMemRelease(&m);
    return SQLITE_OK;
}
