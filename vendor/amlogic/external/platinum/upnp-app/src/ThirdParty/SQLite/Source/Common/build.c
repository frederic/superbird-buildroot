...
void sqlite3BeginParse(Parse *pParse, int explainFlag)
{
     ...
     */
    void sqlite3TableLock(Parse *pParse,  /* Parsing context */
    ...
    static void codeTableLocks(Parse *pParse)
    {
        int i;
        Vdbe *pVdbe;
        assert(sqlite3ThreadDataReadOnly()->useSharedData || pParse->nTableLock
            == 0);
        ...
        for (i = 0; i < pParse->nTableLock; i++)
        {
            ...
            sqlite3VdbeOp3(pVdbe, OP_TableLock, p1, p->iTab, p->zName,
                P3_STATIC);
        }
    }
#else
...
void sqlite3FinishCoding(Parse *pParse)
{
        ...
        if (pParse->cookieGoto > 0)
        {
            ...
            for (iDb = 0, mask = 1; iDb < db->nDb; mask <<= 1, iDb++)
            {
                if ((mask &pParse->cookieMask) == 0)
                    continue;
                sqlite3VdbeAddOp(v, OP_Transaction, iDb, (mask &pParse
                    ->writeMask) != 0);
                sqlite3VdbeAddOp(v, OP_VerifyCookie, iDb, pParse
                    ->cookieValue[iDb]);
            }
             ...
             */
            sqlite3VdbeOp3(v, OP_Noop, 0, 0, pParse->zSql, pParse->zTail -
                pParse->zSql);
    ...
    if (v && pParse->nErr == 0 && !sqlite3MallocFailed())
    {
        ...
        sqlite3VdbeMakeReady(v, pParse->nVar, pParse->nMem + 3, pParse->nTab +
            3, pParse->explain);
...
Table *sqlite3FindTable(sqlite3 *db, const char *zName, const char *zDatabase)
{
    ...
    for (i = OMIT_TEMPDB; i < db->nDb; i++)
    {
        ...
        if (zDatabase != 0 && sqlite3StrICmp(zDatabase, db->aDb[j].zName))
            continue;
        p = sqlite3HashFind(&db->aDb[j].pSchema->tblHash, zName, strlen(zName)
            + 1);
        if (p)
            break;
    }
...
void sqlite3UnlinkAndDeleteIndex(sqlite3 *db, int iDb, const char *zIdxName)
{
    ...
    if (pIndex)
    {
        ...
        {
            Index *p;
            for (p = pIndex->pTable->pIndex; p && p->pNext != pIndex; p = p
                ->pNext) {}
            if (p && p->pNext == pIndex)
            {
...
void sqlite3DeleteTable(sqlite3 *db, Table *pTable)
{
    ...
    for (pIndex = pTable->pIndex; pIndex; pIndex = pNext)
    {
        ...
        for (pFKey = pTable->pFKey; pFKey; pFKey = pNextFKey)
        {
            ...
            assert(sqlite3HashFind(&pTable->pSchema->aFKey, pFKey->zTo, strlen
                (pFKey->zTo) + 1) != pFKey);
            sqliteFree(pFKey);
        }
    #endif
...
void sqlite3UnlinkAndDeleteTable(sqlite3 *db, int iDb, const char *zTabName)
{
    ...
    pDb = &db->aDb[iDb];
    p = sqlite3HashInsert(&pDb->pSchema->tblHash, zTabName, strlen(zTabName) +
        1, 0);
    if (p)
    {
        #ifndef SQLITE_OMIT_FOREIGN_KEY
            for (pF1 = p->pFKey; pF1; pF1 = pF1->pNextFrom)
            {
                ...
                if (pF2 == pF1)
                {
                    sqlite3HashInsert(&pDb->pSchema->aFKey, pF1->zTo, nTo, pF1
                        ->pNextTo);
                }
                else
                {
...
int sqlite3FindDb(sqlite3 *db, Token *pName)
{
    ...
    if (zName)
    {
        ...
        {
            if ((!OMIT_TEMPDB || i != 1) && n == strlen(pDb->zName) && 0 ==
 ...
 */
int sqlite3TwoPartName(Parse *pParse,  /* Parsing and code generating context */
...
{
    if (!pParse->db->init.busy && pParse->nested == 0 && (pParse->db->flags
        &SQLITE_WriteSchema) == 0 && 0 == sqlite3StrNICmp(zName, "sqlite_", 7))
    {
 ...
 */
void sqlite3StartTable(Parse *pParse,  /* Parser context */
...
)
{
    ...
    if (db->init.iDb == 1)
        ...
        assert((isTemp &1) == isTemp);
        {
            ...
            if (sqlite3AuthCheck(pParse, SQLITE_INSERT, SCHEMA_TABLE(isTemp), 0,
                zDb))
            {
                goto begin_table_error;
            }
    ...
    if (!db->init.busy && (v = sqlite3GetVdbe(pParse)) != 0)
    {
        ...
        fileFormat = (db->flags &SQLITE_LegacyFileFmt) != 0 ? 1 :
    ...
    begin_table_error: sqliteFree(zName);
    return ;
}
    ...
    if ((p = pParse->pNewTable) != 0)
    {
        ...
        if (!sqlite3ExprIsConstantOrFunction(pExpr))
        {
            sqlite3ErrorMsg(pParse,
                "default value of column [%s] is not constant", pCol->zName);
        }
        else
        {
            sqlite3ExprDelete(pCol->pDflt);
 ...
 */
void sqlite3AddPrimaryKey(Parse *pParse,  /* Parsing context */
...
)
{
    ...
    else
    {
        for (i = 0; i < pList->nExpr; i++)
        {
            ...
            {
                if (sqlite3StrICmp(pList->a[i].zName, pTab->aCol[iCol].zName)
                    == 0)
                {
                    break;
                }
    ...
    if (zType && sqlite3StrICmp(zType, "INTEGER") == 0 && sortOrder ==
        SQLITE_SO_ASC)
    {
        pTab->iPKey = iCol;
    ...
    primary_key_exit: sqlite3ExprListDelete(pList);
    return ;
}
 ...
 */
void sqlite3AddCheckConstraint(Parse *pParse,  /* Parsing context */
...
)
{
        ...
        if (pTab)
        {
             ...
             ** statement */
            pTab->pCheck = sqlite3ExprAnd(pTab->pCheck, sqlite3ExprDup
                (pCheckExpr));
        }
    #endif
...
CollSeq *sqlite3LocateCollSeq(Parse *pParse, const char *zName, int nName)
{
    ...
    if (!initbusy && (!pColl || !pColl->xCmp))
    {
        ...
        if (!pColl)
        {
            ...
            sqlite3ErrorMsg(pParse, "no such collation sequence: %.*s", nName,
                zName);
            pColl = 0;
        }
    }
...
static void identPut(char *z, int *pIdx, char *zSignedIdent)
{
    ...
    needQuote = zIdent[j] != 0 || isdigit(zIdent[0]) || sqlite3KeywordCode
        (zIdent, j) != TK_ID;
    if (needQuote)
        z[i++] = '"';
    for (j = 0; zIdent[j]; j++)
...
static char *createTableStmt(Table *p, int isTemp)
{
    ...
    if (zStmt == 0)
        return 0;
    strcpy(zStmt, !OMIT_TEMPDB && isTemp ? "CREATE TEMP TABLE " :
        "CREATE TABLE ");
    k = strlen(zStmt);
 ...
 */
void sqlite3EndTable(Parse *pParse,  /* Parse context */
...
)
{
    ...
    if (!db->init.busy)
    {
        ...
        else
        {
            ...
            zStmt = sqlite3MPrintf("CREATE %s %.*s", zType2, n, pParse
                ->sNameToken.z);
        }
         ...
         */
        sqlite3NestedParse(pParse, "UPDATE %Q.%s "
            "SET type='%s', name=%Q, tbl_name=%Q, rootpage=#0, sql=%Q "
            "WHERE rowid=#1", db->aDb[iDb].zName, SCHEMA_TABLE(iDb), zType, p
            ->zName, p->zName, zStmt);
        sqliteFree(zStmt);
            ...
            if (p->autoInc)
            {
                ...
                if (pDb->pSchema->pSeqTab == 0)
                {
                    sqlite3NestedParse(pParse,
                        "CREATE TABLE %Q.sqlite_sequence(name,seq)", pDb->zName)
                        ;
                }
            }
        #endif
    ...
    if (db->init.busy && pParse->nErr == 0)
    {
        ...
        pOld = sqlite3HashInsert(&pSchema->tblHash, p->zName, strlen(p->zName)
            + 1, p);
        if (pOld)
        {
            ...
            for (pFKey = p->pFKey; pFKey; pFKey = pFKey->pNextFrom)
            {
                ...
                pFKey->pNextTo = sqlite3HashFind(&pSchema->aFKey, pFKey->zTo,
                    nTo);
     ...
     */
    void sqlite3CreateView(Parse *pParse,  /* The parsing context */
    ...
    int sqlite3ViewGetColumnNames(Parse *pParse, Table *pTable)
    {
        ...
        if (pTable->nCol < 0)
        {
            sqlite3ErrorMsg(pParse, "view %s is circularly defined", pTable
                ->zName);
            return 1;
        }
    ...
    static void sqliteViewResetAll(sqlite3 *db, int idx)
    {
        ...
        if (!DbHasProperty(db, idx, DB_UnresetViews))
            return ;
        for (i = sqliteHashFirst(&db->aDb[idx].pSchema->tblHash); i; i =
            sqliteHashNext(i))
        {
 ...
 */
    ...
    void sqlite3RootPageMoved(Db *pDb, int iFrom, int iTo)
    {
        ...
        for (pElem = sqliteHashFirst(pHash); pElem; pElem = sqliteHashNext
            (pElem))
        {
...
static void destroyRootPage(Parse *pParse, int iTable, int iDb)
{
    ...
    sqlite3VdbeAddOp(v, OP_Destroy, iTable, iDb);
         ...
         */
        sqlite3NestedParse(pParse,
            "UPDATE %Q.%s SET rootpage=%d WHERE #0 AND rootpage=#0", pParse->db
            ->aDb[iDb].zName, SCHEMA_TABLE(iDb), iTable);
    #endif
}
...
void sqlite3DropTable(Parse *pParse, SrcList *pName, int isView, int noErr)
{
    ...
    if (pTab->readOnly || pTab == db->aDb[iDb].pSchema->pSeqTab)
    {
        ...
        {
            sqlite3ErrorMsg(pParse, "use DROP TABLE to delete table %s", pTab
                ->zName);
            goto exit_drop_table;
        }
        if (!isView && pTab->pSelect)
        {
            sqlite3ErrorMsg(pParse, "use DROP VIEW to delete view %s", pTab
                ->zName);
            goto exit_drop_table;
        }
    #endif
    ...
    if (v)
    {
        ...
        while (pTrigger)
        {
            assert(pTrigger->pSchema == pTab->pSchema || pTrigger->pSchema ==
                db->aDb[1].pSchema);
            ...
            {
                sqlite3NestedParse(pParse,
                    "DELETE FROM %s.sqlite_sequence WHERE name=%Q", pDb->zName,
                    pTab->zName);
            }
        #endif
         ...
         */
        sqlite3NestedParse(pParse,
            "DELETE FROM %Q.%s WHERE tbl_name=%Q and type!='trigger'", pDb
            ->zName, SCHEMA_TABLE(iDb), pTab->zName);
        if (!isView)
        {
    ...
    exit_drop_table: sqlite3SrcListDelete(pName);
}
 ...
 */
void sqlite3CreateForeignKey(Parse *pParse,  /* Parsing context */
...
)
{
        ...
        if (pFromCol == 0)
        {
            ...
            if (pToCol && pToCol->nExpr != 1)
            {
                sqlite3ErrorMsg(pParse, "foreign key on %s"
                    " should reference only one column of table %T", p
                    ->aCol[iCol].zName, pTo);
                goto fk_end;
            }
            nCol = 1;
        ...
        else
        {
            for (i = 0; i < nCol; i++)
            {
                ...
                for (j = 0; j < p->nCol; j++)
                {
                    if (sqlite3StrICmp(p->aCol[j].zName, pFromCol->a[i].zName)
                        == 0)
                    {
...
static void sqlite3RefillIndex(Parse *pParse, Index *pIndex, int memRootPage)
{
    ...
    #ifndef SQLITE_OMIT_AUTHORIZATION
        if (sqlite3AuthCheck(pParse, SQLITE_REINDEX, pIndex->zName, 0, pParse
            ->db->aDb[iDb].zName))
        {
            return ;
        }
 ...
 */
void sqlite3CreateIndex(Parse *pParse,  /* All information about this parse */
...
)
{
    ...
    if (pTblName != 0)
    {
        ...
        if (iDb < 0)
            ...
            if (pName2 && pName2->n == 0 && pTab && pTab->pSchema == db
                ->aDb[1].pSchema)
            {
                iDb = 1;
            }
        ...
        pTab = sqlite3LocateTable(pParse, pTblName->a[0].zName, pTblName
            ->a[0].zDatabase);
        if (!pTab)
            goto exit_create_index;
    ...
    if (pName)
    {
        ...
        if (!db->init.busy)
        {
            ...
            {
                sqlite3ErrorMsg(pParse, "there is already a table named %s",
                    zName);
                goto exit_create_index;
            }
        }
    ...
    else
    {
        ...
        sprintf(zBuf, "_%d", n);
        zName = 0;
        sqlite3SetString(&zName, "sqlite_autoindex_", pTab->zName, zBuf, (char*)
            0);
        if (zName == 0)
            goto exit_create_index;
    }
     ...
     */
    #ifndef SQLITE_OMIT_AUTHORIZATION
        {
            ...
            if (sqlite3AuthCheck(pParse, SQLITE_INSERT, SCHEMA_TABLE(iDb), 0,
                zDb))
            {
                goto exit_create_index;
            }
    ...
    nName = strlen(zName);
    nCol = pList->nExpr;
    pIndex = sqliteMalloc(sizeof(Index) +  /* Index structure  */
    ...
    for (i = 0, pListItem = pList->a; i < pList->nExpr; i++, pListItem++)
    {
        ...
        if (j >= pTab->nCol)
        {
            sqlite3ErrorMsg(pParse, "table %s has no column named %s", pTab
                ->zName, zColName);
            goto exit_create_index;
        }
        pIndex->aiColumn[i] = j;
        ...
        for (pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext)
        {
            ...
            if (k == pIdx->nColumn)
            {
                if (pIdx->onError != pIndex->onError)
                {
                     ...
                     */
                    if (!(pIdx->onError == OE_Default || pIndex->onError ==
                        OE_Default))
                    {
                        sqlite3ErrorMsg(pParse,
    ...
    {
        Index *p;
        p = sqlite3HashInsert(&pIndex->pSchema->idxHash, pIndex->zName, strlen
    ...
    else if (db->init.busy == 0)
    {
        ...
        if (pStart && pEnd)
        {
            ...
            zStmt = sqlite3MPrintf("CREATE%s INDEX %.*s", onError == OE_None ?
                "": " UNIQUE", pEnd->z - pName->z + 1, pName->z);
        }
        else
        {
         ...
         */
        sqlite3NestedParse(pParse,
            "INSERT INTO %Q.%s VALUES('index',%Q,%Q,#0,%Q);", db
            ->aDb[iDb].zName, SCHEMA_TABLE(iDb), pIndex->zName, pTab->zName,
            zStmt);
        ...
        if (pTblName)
        {
            ...
            sqlite3ChangeCookie(db, v, iDb);
            sqlite3VdbeOp3(v, OP_ParseSchema, iDb, 0, sqlite3MPrintf(
                "name='%q'", pIndex->zName), P3_DYNAMIC);
    ...
    {
        if (onError != OE_Replace || pTab->pIndex == 0 || pTab->pIndex->onError
            == OE_Replace)
        {
...
void sqlite3DropIndex(Parse *pParse, SrcList *pName, int ifExists)
{
    ...
    if (v)
    {
        sqlite3NestedParse(pParse, "DELETE FROM %Q.%s WHERE name=%Q", db
            ->aDb[iDb].zName, SCHEMA_TABLE(iDb), pIndex->zName);
        sqlite3ChangeCookie(db, v, iDb);
        destroyRootPage(pParse, pIndex->tnum, iDb);
        sqlite3VdbeOp3(v, OP_DropIndex, iDb, 0, pIndex->zName, 0);
    }
    exit_drop_index: sqlite3SrcListDelete(pName);
}
...
SrcList *sqlite3SrcListAppend(SrcList *pList, Token *pTable, Token *pDatabase)
{
    ...
    } if (pList->nSrc >= pList->nAlloc)
    {
        SrcList *pNew;
        pList->nAlloc *= 2;
        pNew = sqliteRealloc(pList, sizeof(*pList) + (pList->nAlloc - 1)*sizeof
            (pList->a[0]));
        if (pNew == 0)
        {
 ...
 */
    ...
    static void reindexDatabases(Parse *pParse, char const *zColl)
    {
        ...
        {
            assert(pDb != 0);
            for (k = sqliteHashFirst(&pDb->pSchema->tblHash); k; k =
                sqliteHashNext(k))
            {
 ...
 */
    ...
    void sqlite3Reindex(Parse *pParse, Token *pName1, Token *pName2)
    {
        ...
        {
            assert(pName1->z);
            pColl = sqlite3FindCollSeq(db, ENC(db), (char*)pName1->z, pName1->n,
                0);
            if (pColl)
            {
