...
static void attachFunc(sqlite3_context *context, int argc, sqlite3_value **argv)
{
    ...
    {
        sqlite3_snprintf(127, zErr, "too many attached databases - max %d",
            MAX_ATTACHED);
        goto attach_error;
    }
    if (!db->autoCommit)
 ...
 */
static void codeAttach(Parse *pParse,  /* The parser context */
...
)
{
    ...
    sName.pParse = pParse;
    if (SQLITE_OK != (rc = resolveAttachExpr(&sName, pFilename)) || SQLITE_OK
        != (rc = resolveAttachExpr(&sName, pDbname)) || SQLITE_OK != (rc =
        resolveAttachExpr(&sName, pKey)))
    {
        pParse->nErr++;
        goto attach_end;
    }
    ...
    if (v)
    {
        ...
        pFunc = sqlite3FindFunction(db, zFunc, strlen(zFunc), nFunc,
            SQLITE_UTF8, 0);
    ...
    attach_end: sqlite3ExprDelete(pFilename);
...
{
    codeAttach(pParse, SQLITE_DETACH, "sqlite_detach", 1, pDbname, 0, 0,
        pDbname);
}
 ...
 */
int sqlite3FixInit(DbFixer *pFix,  /* The fixer to be initialized */
 ...
 */
int sqlite3FixSrcList(DbFixer *pFix,  /* Context of the fixation */
...
)
{
    ...
    for (i = 0, pItem = pList->a; i < pList->nSrc; i++, pItem++)
    {
        ...
        else if (sqlite3StrICmp(pItem->zDatabase, zDb) != 0)
        {
            sqlite3ErrorMsg(pFix->pParse,
                "%s %T cannot reference objects in database %s", pFix->zType,
                pFix->pName, pItem->zDatabase);
            return 1;
        }
...
#if !defined(SQLITE_OMIT_VIEW) || !defined(SQLITE_OMIT_TRIGGER)
    int sqlite3FixSelect(DbFixer *pFix,  /* Context of the fixation */
    ...
    int sqlite3FixExprList(DbFixer *pFix,  /* Context of the fixation */
