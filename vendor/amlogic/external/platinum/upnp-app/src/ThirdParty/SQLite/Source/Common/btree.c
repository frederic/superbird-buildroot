...
struct BtCursor
{
    ...
    int(*xCompare)(void *, int, const void *, int, const void*); /* Key comp
        func */
...
#define WRITE_LOCK    2
    ...
    static int saveAllCursors(BtShared *pBt, Pgno iRoot, BtCursor *pExcept)
    {
        ...
        if (sqlite3ThreadDataReadOnly()->useSharedData)
        {
            ...
            {
                if (p != pExcept && (0 == iRoot || p->pgnoRoot == iRoot) && p
                    ->eState == CURSOR_VALID)
                {
    ...
    static int restoreOrClearCursorPositionX(BtCursor *pCur, int doSeek)
    {
        ...
        if (rc == SQLITE_OK)
        {
            ...
            pCur->pKey = 0;
            assert(CURSOR_VALID == pCur->eState || CURSOR_INVALID == pCur
                ->eState);
        }
        return rc;
    }
    ...
    static int queryTableLock(Btree *p, Pgno iTab, u8 eLock)
    {
         ...
         */
        if (!p->pSqlite || 0 == (p->pSqlite->flags &SQLITE_ReadUncommitted) ||
            eLock == WRITE_LOCK || iTab == MASTER_ROOT)
        {
            for (pIter = pBt->pLock; pIter; pIter = pIter->pNext)
            {
                if (pIter->pBtree != p && pIter->iTable == iTab && (pIter
                    ->eLock != eLock || eLock != READ_LOCK))
                {
    ...
    static int lockTable(Btree *p, Pgno iTable, u8 eLock)
    {
         ...
         */
        if ((p->pSqlite) && (p->pSqlite->flags &SQLITE_ReadUncommitted) &&
            (eLock == READ_LOCK) && iTable != MASTER_ROOT)
        {
            return SQLITE_OK;
        }
    ...
    static int ptrmapPut(BtShared *pBt, Pgno key, u8 eType, Pgno parent)
    {
        ...
        if (eType != pPtrmap[offset] || get4byte(&pPtrmap[offset + 1]) !=
            parent)
        {
 ...
 */
static void parseCellPtr(MemPage *pPage,  /* Page containing the cell */
...
)
{
    ...
    else
    {
        ...
        surplus = minLocal + (nPayload - minLocal) % (pPage->pBt->usableSize -
            4);
        if (surplus <= maxLocal)
        {
...
static void parseCell(MemPage *pPage,  /* Page containing the cell */
    ...
    static int ptrmapPutOvflPtr(MemPage *pPage, u8 *pCell)
    {
        if (pCell)
        {
            ...
            if ((info.nData + (pPage->intKey ? 0 : info.nKey)) > info.nLocal)
            {
                ...
                return ptrmapPut(pPage->pBt, ovfl, PTRMAP_OVERFLOW1, pPage
                    ->pgno);
            }
        }
        return SQLITE_OK;
 ...
 */
    ...
    static void _pageIntegrity(MemPage *pPage)
    {
        ...
        assert(pPage->aData ==  &((unsigned char*)pPage)[ - pPage->pBt
            ->pageSize]);
        hdr = pPage->hdrOffset;
        ...
        if (pPage->isInit)
        {
            ...
            assert(pPage->hasData == !(pPage->zeroData || (!pPage->leaf &&
                pPage->leafData)));
        ...
        for (i = 0; i < hdr + 10-pPage->leaf *4; i++)
            used[i] = 1;
        nFree = 0;
        ...
        assert(pPage->isInit == 0 || pPage->nFree == nFree + data[hdr + 7] +
            cellLimit - (cellOffset + 2 * nCell));
...
static void freeSpace(MemPage *pPage, int start, int size)
{
    ...
    while ((pbegin = get2byte(&data[addr])) > 0)
    {
        ...
        if (pbegin + psize + 3 >= pnext && pnext > 0)
        {
            ...
            put2byte(&data[pbegin + 2], pnext + get2byte(&data[pnext + 2]) -
                pbegin);
        }
        else
        {
 ...
 */
static int initPage(MemPage *pPage,  /* The page to be initialized */
 ...
 */
int sqlite3BtreeOpen(const char *zFilename,  /* Name of the file containing the
    BTree database */
...
)
{
    ...
    /* Try to find an existing Btree structure opened on zFilename. */
        ...
        if (pTsdro->useSharedData && zFilename && !isMemdb)
        {
            ...
            {
                assert(pBt->nRef > 0);
                if (0 == strcmp(zFullPathname, sqlite3pager_filename(pBt
                    ->pPager)))
                {
                    p->pBt = pBt;
                    *ppBtree = p;
    ...
    if (pBt->pageSize < 512 || pBt->pageSize > SQLITE_MAX_PAGE_SIZE || ((pBt
        ->pageSize - 1) &pBt->pageSize) != 0)
    {
...
int sqlite3BtreeClose(Btree *p)
{
    ...
    sqliteFree(p);
        ...
        {
            BtShared *pPrev;
            for (pPrev = pTsd->pBtree; pPrev && pPrev->pNext != pBt; pPrev =
                pPrev->pNext) {}
            if (pPrev)
            {
                assert(pTsd == sqlite3ThreadData());
...
int sqlite3BtreeSyncDisabled(Btree *p)
{
    ...
    int sqlite3BtreeSetPageSize(Btree *p, int pageSize, int nReserve)
    {
        ...
        if (pageSize >= 512 && pageSize <= SQLITE_MAX_PAGE_SIZE && ((pageSize -
            1) &pageSize) == 0)
        {
            assert((pageSize &7) == 0);
...
static int lockBtree(BtShared *pBt)
{
    ...
    pBt->pPage1 = pPage1;
    return SQLITE_OK;
    page1_init_failed: releasePage(pPage1);
    pBt->pPage1 = 0;
    return rc;
}
...
int sqlite3BtreeBeginTrans(Btree *p, int wrflag)
{
    ...
    static int setChildPtrmaps(MemPage *pPage)
    {
        ...
        set_child_ptrmaps_out: pPage->isInit = isInitOrig;
        return rc;
    }
    ...
    static int modifyPagePointer(MemPage *pPage, Pgno iFrom, Pgno iTo, u8 eType)
    {
        ...
        else
        {
            ...
            if (i == nCell)
            {
                if (eType != PTRMAP_BTREE || get4byte(&pPage->aData[pPage
                    ->hdrOffset + 8]) != iFrom)
                {
                    return SQLITE_CORRUPT_BKPT;
                }
    ...
    )
    {
        ...
        int rc;
        assert(eType == PTRMAP_OVERFLOW2 || eType == PTRMAP_OVERFLOW1 || eType
            == PTRMAP_BTREE || eType == PTRMAP_ROOTPAGE);
    ...
    static int autoVacuumCommit(BtShared *pBt, Pgno *nTrunc)
    {
        ...
        nPtrMap = (nFreeList - origSize + PTRMAP_PAGENO(pBt, origSize) + pgsz /
            5) / (pgsz / 5);
        finSize = origSize - nFreeList - nPtrMap;
        if (origSize > PENDING_BYTE_PAGE(pBt) && finSize <= PENDING_BYTE_PAGE
            (pBt))
        {
            finSize--;
        }
        ...
        *nTrunc = finSize;
        ...
        autovacuum_out: assert(nRef ==  *sqlite3pager_stats(pPager));
        if (rc != SQLITE_OK)
        {
    ...
    void sqlite3BtreeCursorList(Btree *p)
    {
        ...
        for (pCur = pBt->pCursor; pCur; pCur = pCur->pNext)
        {
            ...
            sqlite3DebugPrintf(
                ...
                pCur->pgnoRoot, zMode, pPage ? pPage->pgno: 0, pCur->idx, (pCur
                ->eState == CURSOR_VALID) ? "" : " eof");
        }
    }
#endif
 ...
 */
static int dfltCompare(void *NotUsed,  /* User data is not used */
...
int(*xCmp)(void *, int, const void *, int, const void*),  /* Key Comparison
    func */
void *pArg,  /* First arg to xCompare() */
...
)
{
    ...
    *ppCur = pCur;
    return SQLITE_OK;
    create_cursor_exception: if (pCur)
    {
        releasePage(pCur->pPage);
        sqliteFree(pCur);
    }
     ...
     */
    void sqlite3BtreeSetCompare(BtCursor *pCur,  /* The cursor to whose
        comparison function is changed */
    int(*xCmp)(void *, int, const void *, int, const void*),  /* New comparison
        func */
 ...
 */
static int getPayload(BtCursor *pCur,  /* Cursor pointing to entry to read from
    */
 ...
 */
static const unsigned char *fetchPayload(BtCursor *pCur,  /* Cursor pointing to
    entry to read from */
    ...
    for (;;)
    {
        ...
        while (lwr <= upr)
        {
            ...
            else
            {
                ...
                {
                    c = pCur->xCompare(pCur->pArg, nCellKey, pCellKey, nKey,
                        pKey);
                }
                else
                {
 ...
 */
static int allocatePage(BtShared *pBt, MemPage **ppPage, Pgno *pPgno, Pgno
    nearby, u8 exact)
{
    MemPage *pPage1;
    int rc;
                ...
                else if (searchList && nearby == iTrunk)
                {
                    ...
                    *ppPage = pTrunk;
                    ...
                    else
                    {
                        ...
                        memcpy(&pNewTrunk->aData[8], &pTrunk->aData[12], (k - 1)
                            *4);
                        if (!pPrevTrunk)
                        {
...
)
{
    ...
    *pnSize = info.nSize;
    ...
    while (nPayload > 0)
    {
        ...
        {
            #ifndef SQLITE_OMIT_AUTOVACUUM
                Pgno pgnoPtrmap = pgnoOvfl; /* Overflow page pointer-map entry
                    page */
            #endif
...
static int reparentPage(BtShared *pBt, Pgno pgno, MemPage *pNewParent, int idx)
{
    ...
    if (aData)
    {
        ...
        if (pThis->isInit)
        {
            ...
            {
                if (pThis->pParent)
                    sqlite3pager_unref(pThis->pParent->aData);
 ...
 */
static int insertCell(MemPage *pPage,  /* Page into which we are copying */
    ...
    else
    {
        ...
        pageIntegrity(pPage);
            ...
            if (pPage->pBt->autoVacuum)
            {
                ...
                CellInfo info;
                parseCellPtr(pPage, pCell, &info);
                if ((info.nData + (pPage->intKey ? 0 : info.nKey)) >
                    info.nLocal)
                {
                    Pgno pgnoOvfl = get4byte(&pCell[info.iOverflow]);
                    int rc = ptrmapPut(pPage->pBt, pgnoOvfl, PTRMAP_OVERFLOW1,
                        pPage->pgno);
                    if (rc != SQLITE_OK)
                        return rc;
                }
...
static int balance_nonroot(MemPage *pPage)
{
    ...
    TRACE(("BALANCE: begin page %d child of %d\n", pPage->pgno, pParent->pgno));
         ...
         */
        if (pPage->leaf && pPage->intKey && pPage->leafData && pPage->nOverflow
            == 1 && pPage->aOvfl[0].idx == pPage->nCell && pPage->pParent->pgno
            != 1 && get4byte(&pParent->aData[pParent->hdrOffset + 8]) == pPage
            ->pgno)
        {
            /*
    ...
    if (pParent->idxShift)
    {
        ...
        assert(idx < pParent->nCell || get4byte(&pParent->aData[pParent
            ->hdrOffset + 8]) == pgno);
    }
    else
    {
     ...
     */
    apCell = sqliteMallocRaw(nMaxCells *sizeof(u8*) /* apCell */
    ...
    for (i = 1; i < NB; i++)
    {
        ...
        assert(((aCopy[i] - (u8*)apCell) &7) == 0); /* 8-byte alignment
            required */
    }
    ...
    for (i = k - 1; i > 0; i--)
    {
        ...
        assert(d < nMaxCells);
        assert(r < nMaxCells);
        while (szRight == 0 || szRight + szCell[d] + 2 <= szLeft - (szCell[r] +
            2))
        {
            szRight += szCell[d] + 2;
            szLeft -= szCell[r] + 2;
    ...
    TRACE(("BALANCE: old: %d %d %d  new: %d(%d) %d(%d) %d(%d) %d(%d) %d(%d)\n",
        pgnoOld[0], nOld >= 2 ? pgnoOld[1]: 0, nOld >= 3 ? pgnoOld[2]: 0,
        pgnoNew[0], szNew[0], nNew >= 2 ? pgnoNew[1]: 0, nNew >= 2 ? szNew[1]:
        0, nNew >= 3 ? pgnoNew[2]: 0, nNew >= 3 ? szNew[2]: 0, nNew >= 4 ?
        pgnoNew[3]: 0, nNew >= 4 ? szNew[3]: 0, nNew >= 5 ? pgnoNew[4]: 0, nNew
        >= 5 ? szNew[4]: 0));
    /*
    ...
    for (i = 0; i < nNew; i++)
    {
        ...
        assert(pNew->nOverflow == 0);
            ...
            if (pBt->autoVacuum)
            {
                ...
                {
                    assert(k < nMaxCells);
                    if (aFrom[k] == 0xFF || apCopy[aFrom[k]]->pgno != pNew
                        ->pgno)
                    {
     ...
     */
    balance_cleanup: sqliteFree(apCell);
    for (i = 0; i < nOld; i++)
    {
        releasePage(apOld[i]);
    }
...
static int balance_shallower(MemPage *pPage)
{
    ...
    else
    {
        ...
        if (pPage->pgno == 1)
        {
            ...
            if (pChild->nFree >= 100)
            {
                ...
                put4byte(&pPage->aData[pPage->hdrOffset + 8], get4byte(&pChild
                    ->aData[pChild->hdrOffset + 8]));
                freePage(pChild);
    ...
    end_shallow_balance: sqliteFree(apCell);
    return rc;
}
...
static int balance_deeper(MemPage *pPage)
{
    ...
    memcpy(pChild->aOvfl, pPage->aOvfl, pPage->nOverflow *sizeof(pPage
        ->aOvfl[0]));
    ...
    balancedeeper_out: releasePage(pChild);
    return rc;
}
...
static int balance(MemPage *pPage, int insert)
{
    ...
    {
        if (pPage->nOverflow > 0 || (!insert && pPage->nFree > pPage->pBt
            ->usableSize *2 / 3))
        {
...
static int checkReadLocks(BtShared *pBt, Pgno pgnoRoot, BtCursor *pExclude)
{
    ...
    for (p = pBt->pCursor; p; p = p->pNext)
    {
        ...
        if (p->pgnoRoot != pgnoRoot || p == pExclude)
            continue;
        if (p->wrFlag == 0 && flags &SQLITE_ReadUncommitted)
            continue;
        if (p->wrFlag == 0)
            return SQLITE_LOCKED;
 ...
 */
int sqlite3BtreeInsert(BtCursor *pCur,  /* Insert data into the table of this
    cursor */
...
)
{
    ...
    TRACE(("INSERT: table=%d nkey=%lld ndata=%d page=%d %s\n", pCur->pgnoRoot,
        nKey, nData, pPage->pgno, loc == 0 ? "overwrite" : "new entry"));
    assert(pPage->isInit);
    rc = sqlite3pager_write(pPage->aData);
    if (rc)
        return rc;
...
int sqlite3BtreeDelete(BtCursor *pCur)
{
     ...
     */
    if ((rc = restoreOrClearCursorPosition(pCur, 1)) != 0 || (rc =
        saveAllCursors(pBt, pCur->pgnoRoot, pCur)) != 0 || (rc =
        sqlite3pager_write(pPage->aData)) != 0)
    {
        return rc;
    }
    ...
    if (!pPage->leaf)
    {
        ...
        {
            TRACE((
                "DELETE: table=%d delete internal from %d replace from leaf %d\n", pCur->pgnoRoot, pPage->pgno, leafCur.pPage->pgno));
...
int sqlite3BtreeCreateTable(Btree *p, int *piTable, int flags)
{
    ...
    if (pBt->pCursor)
    {
        ...
        if (pBt->autoVacuum)
        {
             ...
             ** PENDING_BYTE page.
             */
            if (pgnoRoot == PTRMAP_PAGENO(pBt, pgnoRoot) || pgnoRoot ==
            ...
            if (pgnoMove != pgnoRoot)
            {
                ...
                if (rc != SQLITE_OK || eType == PTRMAP_ROOTPAGE || eType ==
                    PTRMAP_FREEPAGE)
                {
                    releasePage(pRoot);
                    return rc;
 ...
 */
static int clearDatabasePage(BtShared *pBt,  /* The BTree that contains the
    table */
...
)
{
    ...
    if (!pPage->leaf)
    {
        rc = clearDatabasePage(pBt, get4byte(&pPage->aData[8]), pPage->pParent,
            1);
        if (rc)
            goto cleardatabasepage_out;
    }
...
int sqlite3BtreeFlags(BtCursor *pCur)
{
     ...
     */
    static int btreePageDump(BtShared *pBt, int pgno, int recursive, MemPage
        *pParent)
    {
        int rc;
        MemPage *pPage;
        ...
        sqlite3DebugPrintf("PAGE %d:  flags=0x%02x  frag=%d   parent=%d\n",
            pgno, data[hdr], data[hdr + 7], (pPage->isInit && pPage->pParent) ?
            pPage->pParent->pgno: 0);
        ...
        for (i = 0; i < nCell; i++)
        {
            ...
            sqlite3DebugPrintf(
                ...
                range, child, info.nKey, info.nData, payload);
        }
        if (!pPage->leaf)
        {
...
struct IntegrityCk
{
     ...
     */
    static void checkAppendMsg(IntegrityCk *pCheck, char *zMsg1, const char
        *zFormat, ...)
    {
        va_list ap;
        char *zMsg2;
    ...
    static int checkRef(IntegrityCk *pCheck, int iPage, char *zContext)
    {
         ...
         ** to pCheck.
         */
        static void checkPtrmap(IntegrityCk *pCheck,  /* Integrity check
            context */
        ...
        )
        {
            ...
            if (rc != SQLITE_OK)
            {
                checkAppendMsg(pCheck, zContext, "Failed to read ptrmap key=%d",
                    iChild);
                return ;
            }
                ...
                else
                {
                    for (i = 0; i < n; i++)
                    {
                        Pgno iFreePage = get4byte(&pOvfl[8+i * 4]);
                            ...
                            if (pCheck->pBt->autoVacuum)
                            {
                                checkPtrmap(pCheck, iFreePage, PTRMAP_FREEPAGE,
                                    0, zContext);
                            }
                        #endif
     ...
     */
    static int checkTreePage(IntegrityCk *pCheck,  /* Context for the sanity
        check */
    ...
    )
    {
        ...
        {
            checkAppendMsg(pCheck, zContext, "initPage() returns error code %d",
                rc);
            releasePage(pPage);
            return 0;
        }
        ...
        for (i = 0; i < pPage->nCell; i++)
        {
            ...
            if (sz > info.nLocal)
            {
                int nPage = (sz - info.nLocal + usableSize - 5) / (usableSize -
                    4);
                Pgno pgnoOvfl = get4byte(&pCell[info.iOverflow]);
                #ifndef SQLITE_OMIT_AUTOVACUUM
                    if (pBt->autoVacuum)
        ...
        if (hit)
        {
            ...
            for (cnt = 0, i = get2byte(&data[hdr + 1]); i > 0 && i < usableSize
                && cnt < 10000; cnt++)
            {
    ...
    char *sqlite3BtreeIntegrityCheck(Btree *p, int *aRoot, int nRoot)
    {
        ...
        sCheck.anRef = sqliteMallocRaw((sCheck.nPage + 1) *sizeof
            (sCheck.anRef[0]));
        if (!sCheck.anRef)
        {
            unlockBtreeIfUnused(pBt);
            return sqlite3MPrintf("Unable to malloc %d bytes", (sCheck.nPage +
                1) *sizeof(sCheck.anRef[0]));
        }
         ...
         */
        checkList(&sCheck, 1, get4byte(&pBt->pPage1->aData[32]), get4byte(&pBt
            ->pPage1->aData[36]), "Main freelist: ");
        ...
        for (i = 1; i <= sCheck.nPage; i++)
        {
                 ...
                 */
                if (sCheck.anRef[i] == 0 && (PTRMAP_PAGENO(pBt, i) != i || !pBt
                    ->autoVacuum))
                {
                    checkAppendMsg(&sCheck, 0, "Page %d is never used", i);
                }
        ...
        {
            checkAppendMsg(&sCheck, 0,
                "Outstanding page count goes from %d to %d during this analysis", nRef,  *sqlite3pager_stats(pBt->pPager));
        }
 ...
 */
    ...
    #include <tcl.h>
    int sqlite3_shared_cache_report(void *clientData, Tcl_Interp *interp, int
        objc, Tcl_Obj *CONST objv[])
    {
            ...
            if (pTd->useSharedData)
            {
                ...
                for (pBt = pTd->pBtree; pBt; pBt = pBt->pNext)
                {
                    ...
                    Tcl_ListObjAppendElement(interp, pRet, Tcl_NewStringObj
                        (zFile,  - 1));
                    Tcl_ListObjAppendElement(interp, pRet, Tcl_NewIntObj(pBt
                        ->nRef));
                }
