...
int sqlite3VdbeMemNulTerminate(Mem *pMem)
{
     ...
     */
    assert(!(pMem->flags &MEM_Str) ||  /* it's not a string, or      */
 ...
 */
int sqlite3VdbeMemSetStr(Mem *pMem,  /* Memory cell to set to string value */
...
)
{
    ...
    pMem->n = n;
    assert(enc == 0 || enc == SQLITE_UTF8 || enc == SQLITE_UTF16LE || enc ==
        SQLITE_UTF16BE);
    switch (enc)
    {
        case 0:
...
int sqlite3MemCompare(const Mem *pMem1, const Mem *pMem2, const CollSeq *pColl)
{
    ...
    if (combined_flags &MEM_Str)
    {
        ...
        assert(pMem1->enc == SQLITE_UTF8 || pMem1->enc == SQLITE_UTF16LE ||
        ...
        if (pColl)
        {
            ...
            else
            {
                ...
                n1 = v1 == 0 ? 0 : pMem1->n;
                assert(n1 == sqlite3ValueBytes((sqlite3_value*)pMem1, pColl
                    ->enc));
                v2 = sqlite3ValueText((sqlite3_value*)pMem2, pColl->enc);
                n2 = v2 == 0 ? 0 : pMem2->n;
                assert(n2 == sqlite3ValueBytes((sqlite3_value*)pMem2, pColl
                    ->enc));
                /* Do the comparison */
 ...
 */
int sqlite3VdbeMemFromBtree(BtCursor *pCur,  /* Cursor pointing at record to
    retrieve. */
...
)
{
    ...
    void sqlite3VdbeMemSanity(Mem *pMem)
    {
        ...
        if (pMem->flags &(MEM_Str | MEM_Blob))
        {
            ...
            assert((x &(x - 1)) == 0); /* Only one string subtype can be
                defined */
            ...
            if ((flags &MEM_Str))
            {
                assert(pMem->enc == SQLITE_UTF8 || pMem->enc == SQLITE_UTF16BE
        ...
        else
        {
            ...
            assert((pMem->flags &(MEM_Static | MEM_Dyn | MEM_Ephem | MEM_Short))
                == 0);
            assert(pMem->xDel == 0);
        }
 ...
 */
int sqlite3ValueFromExpr(Expr *pExpr, u8 enc, u8 affinity, sqlite3_value
    **ppVal)
{
    int op;
    char *zVal = 0;
    ...
    {
        if (SQLITE_OK == sqlite3ValueFromExpr(pExpr->pLeft, enc, affinity,
            &pVal))
        {
            pVal->i =  - 1 * pVal->i;
            pVal->r =  - 1.0 * pVal->r;
        ...
        else if (op == TK_BLOB)
        {
            ...
            sqlite3Dequote(zVal);
            nVal = strlen(zVal) / 2;
            sqlite3VdbeMemSetStr(pVal, sqlite3HexToBlob(zVal), nVal, 0,
                sqlite3FreeX);
            sqliteFree(zVal);
        }
    #endif
 ...
 */
void sqlite3ValueSetStr(sqlite3_value *v, int n, const void *z, u8 enc, void
    (*xDel)(void*))
{
    if (v)
