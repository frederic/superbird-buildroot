...
static void applyNumericAffinity(Mem *pRec)
{
    if ((pRec->flags &(MEM_Real | MEM_Int)) == 0)
    {
        ...
        if ((pRec->flags &MEM_Str) && sqlite3IsNumber(pRec->z, &realnum, pRec
            ->enc))
        {
            i64 value;
...
)
{
    ...
    for (pc = p->pc; rc == SQLITE_OK; pc++)
    {
        ...
        switch (pOp->opcode)
        {
            ...
            case OP_Gosub:
                 /* no-push */
                {
                    assert(p->returnDepth < sizeof(p->returnStack) / sizeof(p
                        ->returnStack[0]));
            ...
            case OP_String8:
                 /* same as TK_STRING */
                {
                    ...
                    pOp->p1 = strlen(pOp->p3);
                        ...
                        {
                            pTos++;
                            sqlite3VdbeMemSetStr(pTos, pOp->p3,  - 1,
                                SQLITE_UTF8, SQLITE_STATIC);
                            if (SQLITE_OK != sqlite3VdbeChangeEncoding(pTos,
                                encoding))
                                goto no_mem;
                            if (SQLITE_OK != sqlite3VdbeMemDynamicify(pTos))
                                goto no_mem;
                            pTos->flags &= ~(MEM_Dyn);
            ...
            case OP_Concat:
                 ...
                 */
            case OP_Add:
                 /* same as TK_PLUS, no-push */
            case OP_Subtract:
                 /* same as TK_MINUS, no-push */
            case OP_Multiply:
                 /* same as TK_STAR, no-push */
            case OP_Divide:
                 /* same as TK_SLASH, no-push */
            case OP_Remainder:
                 /* same as TK_REM, no-push */
                {
                    Mem *pNos = &pTos[ - 1];
                    int flags;
            ...
            case OP_Function:
                {
                    ...
                    assert(pOp->p3type == P3_FUNCDEF || pOp->p3type ==
                        P3_VDBEFUNC);
                 ...
                 */
            case OP_BitAnd:
                 /* same as TK_BITAND, no-push */
            case OP_BitOr:
                 /* same as TK_BITOR, no-push */
            case OP_ShiftLeft:
                 /* same as TK_LSHIFT, no-push */
            case OP_ShiftRight:
                 /* same as TK_RSHIFT, no-push */
                {
                    Mem *pNos = &pTos[ - 1];
                    i64 a, b;
                    assert(pNos >= p->aStack);
            ...
            case OP_ToInt:
                 ...
                 */
            case OP_Eq:
                 /* same as TK_EQ, no-push */
            case OP_Ne:
                 /* same as TK_NE, no-push */
            case OP_Lt:
                 /* same as TK_LT, no-push */
            case OP_Le:
                 /* same as TK_LE, no-push */
            case OP_Gt:
                 /* same as TK_GT, no-push */
            case OP_Ge:
                 /* same as TK_GE, no-push */
                {
                    Mem *pNos;
                    int flags;
                    int res;
                    ...
                    if (flags &MEM_Null)
                    {
                        if ((pOp->p1 &0x200) != 0)
                        {
                             ...
                             */
                            sqlite3VdbeMemSetInt64(pTos, (pTos->flags &MEM_Null)
                                == 0);
                            sqlite3VdbeMemSetInt64(pNos, (pNos->flags &MEM_Null)
                                == 0);
                        }
                        else
                        {
                 ...
                 */
            case OP_And:
                 /* same as TK_AND, no-push */
            case OP_Or:
                 /* same as TK_OR, no-push */
                {
                    Mem *pNos = &pTos[ - 1];
                 ...
                 */
            case OP_Negative:
                 /* same as TK_UMINUS, no-push */
            case OP_AbsValue:
                {
                    assert(pTos >= p->aStack);
                    if (pTos->flags &MEM_Real)
                    {
                        neg_abs_real_case: Release(pTos);
            ...
            case OP_NotNull:
                 /* same as TK_NOTNULL, no-push */
                {
                    ...
                    for (i = 0; i < cnt && (pTos[1+i - cnt].flags &MEM_Null) ==
                        0; i++) {}
                    if (i >= cnt)
                        pc = pOp->p2 - 1;
                    if (pOp->p1 > 0)
            ...
            case OP_Column:
                {
                    ...
                    u32 *aType; /* aType[i] holds the numeric type of the i-th
                        column */
                    u32 *aOffset; /* aOffset[i] is offset to start of data for
                        i-th column */
                    u32 nField; /* number of fields in the record */
                    int len; /* The length of the serialized data for the
                        column */
                    int i; /* Loop counter */
                    ...
                    else
                    {
                        ...
                        int szHdrSz; /* Size of the header size field at start
                            of record */
                        int avail; /* Number of bytes of available data */
                        aType = pC->aType;
                        if (aType == 0)
                        {
                            pC->aType = aType = sqliteMallocRaw(2 *nField *
                                sizeof(aType));
                        }
                        if (aType == 0)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                zData = (char*)sqlite3BtreeDataFetch(pCrsr,
                                    &avail);
                            }
                        ...
                        {
                            rc = sqlite3VdbeMemFromBtree(pCrsr, 0, offset, pC
                                ->isIndex, &sMem);
                            if (rc != SQLITE_OK)
                            {
                                goto op_column_out;
                    ...
                    else
                    {
                        ...
                        {
                            sqlite3VdbeMemShallowCopy(pTos, (Mem*)(pOp->p3),
                                MEM_Static);
                        }
                        else
                        {
            ...
            case OP_MakeRecord:
                {
                    ...
                    int containsNull = 0; /* True if any of the data fields are
                        NULL */
                    char zTemp[NBFS]; /* Space to hold small records */
                    Mem *pData0;
                    int leaveOnStack; /* If true, leave the entries on the
                        stack */
                    int nField; /* Number of fields in the record */
                    int jumpIfNull; /* Jump here if non-zero and any entries
                        are NULL. */
                    ...
                    {
                        serial_type = sqlite3VdbeSerialType(pRec, file_format);
                        zCsr += sqlite3PutVarint(zCsr, serial_type); /* serial
                            type */
                    }
                    if (addRowid)
                    {
                        zCsr += sqlite3PutVarint(zCsr, sqlite3VdbeSerialType
                            (pRowid, 0));
                    }
                    for (pRec = pData0; pRec <= pTos; pRec++)
                    {
                    ...
                    pTos->enc = SQLITE_UTF8; /* In case the blob is ever
                        converted to text */
            ...
            case OP_AutoCommit:
                 /* no-push */
                {
                    ...
                    assert(db->activeVdbeCnt > 0); /* At least this one VM is
                        active */
                    if (db->activeVdbeCnt > 1 && i && !db->autoCommit)
                    {
                         ...
                         */
                        sqlite3SetString(&p->zErrMsg, "cannot ", rollback ?
                            "rollback" : "commit",
                            " transaction - SQL statements in progress", (char*)
                            0);
                        rc = SQLITE_ERROR;
                    }
                    ...
                    {
                        sqlite3SetString(&p->zErrMsg, (!i) ?
                            "cannot start a transaction within a transaction" :
                            ((rollback) ?
                            "cannot rollback - no transaction is active" :
                            "cannot commit - no transaction is active"), (char*)
                            0);
                        rc = SQLITE_ERROR;
                    }
                    break;
            ...
            case OP_Transaction:
                 /* no-push */
                {
                    ...
                    if (pBt)
                    {
                        ...
                        if (rc != SQLITE_OK && rc != SQLITE_READONLY /* && rc
                            !=SQLITE_BUSY */)
                        {
                            goto abort_due_to_error;
                        }
            ...
            case OP_ReadCookie:
                {
                     ...
                     */
                    rc = sqlite3BtreeGetMeta(db->aDb[pOp->p1].pBt, 1+pOp->p2,
                        (u32*) &iMeta);
                    pTos++;
                    pTos->i = iMeta;
                    pTos->flags = MEM_Int;
            ...
            case OP_SetCookie:
                 /* no-push */
                {
                    ...
                    rc = sqlite3BtreeUpdateMeta(pDb->pBt, 1+pOp->p2, (int)pTos
                        ->i);
                    if (pOp->p2 == 0)
                    {
                 ...
                 /* no-push */
                {
                    ...
                    if (rc == SQLITE_OK && iMeta != pOp->p2)
                    {
                        sqlite3SetString(&p->zErrMsg,
                            "database schema has changed", (char*)0);
                        rc = SQLITE_SCHEMA;
                    }
                    break;
                }
            ...
            case OP_OpenWrite:
                 /* no-push */
                {
                    ...
                    rc = sqlite3BtreeCursor(pX, p2, wrFlag,
                        sqlite3VdbeRecordCompare, pOp->p3,  &pCur->pCursor);
                    ...
                    case SQLITE_BUSY:
                        {
                            p->pc = pc;
                            p->rc = SQLITE_BUSY;
                            p->pTos = &pTos[1+(pOp->p2 <= 0)]; /* Operands must
                                remain on stack */
                            return SQLITE_BUSY;
                        }
                    case SQLITE_OK:
                        ...
                        if (pOp->p3)
                        {
                            ...
                            if (rc == SQLITE_OK)
                            {
                                ...
                                rc = sqlite3BtreeCursor(pCx->pBt, pgno, 1,
                                    sqlite3VdbeRecordCompare, pOp->p3, &pCx
                                    ->pCursor);
                    ...
                    if (pC->pCursor != 0)
                    {
                        ...
                        *pC->pIncrKey = oc == OP_MoveGt || oc == OP_MoveLe;
                        if (pC->isTable)
                        {
                            ...
                            rc = sqlite3BtreeMoveto(pC->pCursor, 0, (u64)iKey,
                                &res);
                            if (rc != SQLITE_OK)
                            {
                                goto abort_due_to_error;
                        ...
                        else
                        {
                            ...
                            rc = sqlite3BtreeMoveto(pC->pCursor, pTos->z, pTos
                                ->n, &res);
                            if (rc != SQLITE_OK)
                            {
                                goto abort_due_to_error;
            ...
            case OP_Found:
                 /* no-push */
                {
                    ...
                    if ((pC = p->apCsr[i])->pCursor != 0)
                    {
                        ...
                        assert(pC->isTable == 0);
                        Stringify(pTos, encoding);
                        rx = sqlite3BtreeMoveto(pC->pCursor, pTos->z, pTos->n,
                            &res);
            ...
            case OP_IsUnique:
                 /* no-push */
                {
                    ...
                    {
                        int res;
                        i64 v; /* The record number on the P1 entry that
                            matches K */
                        char *zKey; /* The value of K */
                        int nKey; /* Number of bytes in K */
                        int len; /* Number of bytes in K without the rowid at
                            the end */
                        int szRowid; /* Size of the rowid column at the end of
                            zKey */
            ...
            case OP_NewRowid:
                {
                    ...
                    else
                    {
                        ...
                        i64 x;
                        cnt = 0;
                        if ((sqlite3BtreeFlags(pC->pCursor) &(BTREE_INTKEY |
                            BTREE_ZERODATA)) != BTREE_INTKEY)
                        {
                            rc = SQLITE_CORRUPT_BKPT;
                            goto abort_due_to_error;
                        }
                        ...
                        assert((sqlite3BtreeFlags(pC->pCursor) &BTREE_ZERODATA)
                             ...
                             */
                            #define MAX_ROWID  ( (((u64)0x7fffffff)<<32) | (u64)
                                0xffffffff )
                        #endif
                            ...
                            else
                            {
                                ...
                                if (pOp->p2)
                                {
                                    ...
                                    assert(pOp->p2 > 0 && pOp->p2 < p->nMem);
                                        /* P2 is a valid memory cell */
                                    pMem = &p->aMem[pOp->p2];
                                    sqlite3VdbeMemIntegerify(pMem);
                                    assert((pMem->flags &MEM_Int) != 0); /* mem
                                        (P2) holds an integer */
                                    if (pMem->i == MAX_ROWID || pC
                                        ->useRandomRowid)
                                    {
                                        rc = SQLITE_FULL;
                                        goto abort_due_to_error;
                        ...
                        if (pC->useRandomRowid)
                        {
                            assert(pOp->p2 == 0); /* SQLITE_FULL must have
                            ...
                            do
                            {
                                ...
                                if (v == 0)
                                    continue;
                                x = intToKey(v);
                                rx = sqlite3BtreeMoveto(pC->pCursor, 0, (u64)x,
                                    &res);
                                cnt++;
                            }
            ...
            case OP_Insert:
                 /* no-push */
                {
                    ...
                    {
                        i64 iKey; /* The integer ROWID or key for the record to
                            be inserted */
                        ...
                        {
                            rc = sqlite3BtreeInsert(pC->pCursor, 0, iKey, pTos
                                ->z, pTos->n);
                        }
                        pC->rowidIsValid = 0;
                        pC->deferredMoveto = 0;
                        ...
                        if (rc == SQLITE_OK && db->xUpdateCallback && pOp->p3)
                        {
                            ...
                            int op = ((pOp->p2 &OPFLAG_ISUPDATE) ?
                                SQLITE_UPDATE : SQLITE_INSERT);
                            assert(pC->isTable);
                            db->xUpdateCallback(db->pUpdateArg, op, zDb, zTbl,
                                iKey);
                            assert(pC->iDb >= 0);
                        }
                    }
            ...
            case OP_RowData:
                {
                    ...
                    pTos->enc = SQLITE_UTF8; /* In case the blob is ever cast
                        to text */
                    break;
                }
                /* Opcode: Rowid P1 * *
                 ...
                 /* no-push */
                {
                    ...
                    if ((pCrsr = pC->pCursor) != 0)
                    {
                        ...
                        else
                        {
                            ...
                            rc = pOp->opcode == OP_Next ? sqlite3BtreeNext
                                (pCrsr, &res): sqlite3BtreePrevious(pCrsr, &res)
                                ;
                            pC->nullRow = res;
                            pC->cacheStatus = CACHE_STALE;
                        }
            ...
            case OP_IdxGE:
                 /* no-push */
                {
                    ...
                    {
                        int res;
                        assert(pTos->flags &MEM_Blob); /* Created using
                            OP_Make*Key */
                        Stringify(pTos, encoding);
                        assert(pC->deferredMoveto == 0);
                        *pC->pIncrKey = pOp->p3 != 0;
                        assert(pOp->p3 == 0 || pOp->opcode != OP_IdxGT);
                        rc = sqlite3VdbeIdxKeyCompare(pC, pTos->n, (u8*)pTos->z,
                            &res);
                        *pC->pIncrKey = 0;
                        if (rc != SQLITE_OK)
                        {
            ...
            case OP_Destroy:
                {
                    ...
                    else
                    {
                        ...
                        rc = sqlite3BtreeDropTable(db->aDb[pOp->p2].pBt, pOp
                            ->p1, &iMoved);
                        pTos++;
                        pTos->flags = MEM_Int;
                        pTos->i = iMoved;
                            ...
                            {
                                sqlite3RootPageMoved(&db->aDb[pOp->p2], iMoved,
                                    pOp->p1);
                            }
                        #endif
                    }
                 ...
                 /* no-push */
                {
                     ...
                     */
                        ...
                        if (db->xUpdateCallback && pOp->p3)
                        {
                            ...
                            BtCursor *pCur = 0;
                            int fin = 0;
                            rc = sqlite3BtreeCursor(pBt, pOp->p1, 0, 0, 0,
                                &pCur);
                            if (rc != SQLITE_OK)
                            {
                                goto abort_due_to_error;
                            }
                            for (rc = sqlite3BtreeFirst(pCur, &fin); rc ==
                                SQLITE_OK && !fin; rc = sqlite3BtreeNext(pCur,
                                &fin))
                            {
                                i64 iKey;
            ...
            case OP_ParseSchema:
                ...
                #if !defined(SQLITE_OMIT_ANALYZE) && !defined
                    (SQLITE_OMIT_PARSER)
                ...
                #endif /* !defined(SQLITE_OMIT_ANALYZE) && !defined
                    (SQLITE_OMIT_PARSER)  */
                ...
                case OP_IntegrityCk:
                    {
                        ...
                        popStack(&pTos, nRoot);
                        pTos++;
                        z = sqlite3BtreeIntegrityCheck(db->aDb[pOp->p2].pBt,
                            aRoot, nRoot);
                        if (z == 0 || z[0] == 0)
                        {
                            if (z)
            ...
            case OP_FifoRead:
                ...
                case OP_ContextPush:
                     /* no-push */
                    {
                        ...
                        if (i >= p->contextStackDepth)
                        {
                            ...
                            sqliteReallocOrFree((void **) &p->contextStack,
                                sizeof(Context)*(i + 1));
                            if (p->contextStack == 0)
                                goto no_mem;
                        }
                ...
                case OP_ContextPop:
                     /* no-push */
                    {
                        Context *pContext = &p->contextStack[--p
                            ->contextStackTop];
            ...
            case OP_MemMove:
                {
                    assert(pOp->p1 >= 0 && pOp->p1 < p->nMem);
                    assert(pOp->p2 >= 0 && pOp->p2 < p->nMem);
                    rc = sqlite3VdbeMemMove(&p->aMem[pOp->p1], &p->aMem[pOp
                        ->p2]);
                    break;
                }
            ...
            case OP_Expire:
                ...
                case OP_TableLock:
                     /* no-push */
                    {
                        ...
                        rc = sqlite3BtreeLockTable(db->aDb[p1].pBt, pOp->p2,
                            isWriteLock);
                        if (rc == SQLITE_LOCKED)
                        {
                            const char *z = (const char*)pOp->p3;
                            sqlite3SetString(&p->zErrMsg,
                                "database table is locked: ", z, (char*)0);
                        }
                        break;
                    }
            ...
            assert(pc >=  - 1 && pc < p->nOp);
                ...
                if (p->trace && pTos >= p->aStack)
                {
                    ...
                    for (i = 0; i >  - 5 &&  &pTos[i] >= p->aStack; i--)
                    {
                        ...
                        else if ((pTos[i].flags &(MEM_Int | MEM_Str)) ==
                            (MEM_Int | MEM_Str))
                        {
     ...
     */
    no_mem: sqlite3SetString(&p->zErrMsg, "out of memory", (char*)0);
    rc = SQLITE_NOMEM;
    goto vdbe_halt;
    /* Jump to here for an SQLITE_MISUSE error.
     */
    abort_due_to_misuse: rc = SQLITE_MISUSE;
    /* Fall thru into abort_due_to_error */
    /* Jump to here for any other kind of fatal error.  The "rc" variable
     ** should hold the error number.
     */
    abort_due_to_error: if (p->zErrMsg == 0)
    {
        if (sqlite3MallocFailed())
            rc = SQLITE_NOMEM;
     ...
     ** flag.
     */
    abort_due_to_interrupt: assert(db->flags &SQLITE_Interrupt);
