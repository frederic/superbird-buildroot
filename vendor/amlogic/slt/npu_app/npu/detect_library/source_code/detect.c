...
typedef det_status_t (*network_create)(const char * data_file_path, dev_type type, void** context);
...
typedef void (*network_release)(int flag);

typedef det_status_t (*network_setparam)(det_model_type modelType,det_param_t param);
typedef det_status_t (*network_getparam)(det_model_type modelType,det_param_t *param);
...
typedef struct function_process {
	...
	network_setparam    model_setparam;
	network_getparam    model_getparam;
}network_process;
...
const char * so_file_name[DET_BUTT]= {
	...
	"libnn_deeplab_v3.so",
	"libnn_facenet.so",
	"libnn_aml_face_recognition.so",
	"libnn_aml_face_detection.so",
};
...
static void check_and_set_dev_type()
{
	...
	char res = buffer[index+3];

	if (index == -1) {
		index = find_string_index(buffer, "2b0", length);
		if (index != -1) {
			res = 'c';
		}
	}
	...
	switch (res) {
		...
		case 'c':
			...
			break;
		default:
			LOGI("set_dev_type DEV_A1 and setenv 3 index=%d char:%c", index, buffer[index+3]);
			g_dev_type = DEV_A1;
			setenv("DEV_TYPE", "3", 0);
			break;
	}
...
det_status_t check_and_set_function(det_model_type modelType)
{
	...
	net->process.model_setparam = (network_setparam)dlsym(net->handle_id, "model_setparam");
	_CHECK_STATUS_(net->process.model_setparam, exit);

	net->process.model_getparam = (network_getparam)dlsym(net->handle_id, "model_getparam");
	_CHECK_STATUS_(net->process.model_getparam, exit);
...
det_status_t det_set_model(det_model_type modelType)
{
	...
	ret = net->process.model_create(data_file_path, g_dev_type, &g_context);
	if (ret) {
...
det_status_t det_set_param(det_model_type modelType, det_param_t param)
{
	...
	ret = net->process.model_setparam(modelType, param);
	if (ret) {
		LOGW("inviald detect_number:%d , Min:%x,Max:%d",  param.param.det_param.detect_num, DETECT_NUM, MAX_DETECT_NUM);
		ret = DET_STATUS_PARAM_ERROR;
	}
exit:
	LOGP("Leave, modeltype:%d", modelType);
	return ret;
...
det_status_t det_get_param(det_model_type modelType,det_param_t *param)
{
	...
	net->process.model_getparam(modelType, param);
...
{
	int i = 0, count = 0;
	p_det_network_t net = NULL;

	for (i=0; i<DET_BUTT; i++) {
		net = &network[i];
		if (net->status) {
			count++;
		}
...
det_status_t det_release_model(det_model_type modelType)
{
	...
	if (check_context()>1) {
		net->process.model_release(0);
	} else {
		net->process.model_release(1);
		g_context = NULL;
	}
