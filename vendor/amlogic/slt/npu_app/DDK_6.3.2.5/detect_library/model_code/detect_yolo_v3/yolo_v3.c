...
det_status_t model_create(const char * data_file_path, dev_type type)
{
	...
	switch (type) {
		case DEV_REVA:
			sprintf(model_path, "%s%s", data_file_path, "/yolov3_7D.export.data");
			g_graph = vnn_CreateYolov37d(model_path, NULL);
			break;
		case DEV_REVB:
			sprintf(model_path, "%s%s", data_file_path, "/yolov3_88.export.data");
			g_graph = vnn_CreateYolov388(model_path, NULL);
			break;
		default:
			break;
	}
...
void model_release(dev_type type)
{
	switch (type) {
		case DEV_REVA:
			vnn_ReleaseYolov37d(g_graph, TRUE);
			break;
		case DEV_REVB:
			vnn_ReleaseYolov388(g_graph, TRUE);
			break;
		default:
			break;
	}
