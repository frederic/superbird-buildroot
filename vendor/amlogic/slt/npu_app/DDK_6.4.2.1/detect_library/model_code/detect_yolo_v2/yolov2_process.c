...
static void get_detections_result(pDetResult resultData, int num, float thresh, box *boxes, float **probs, char **names, int classes)
{
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (prob > thresh) {
            ...
            resultData->result.det_result.point[detect_num].type = DET_RECTANGLE_TYPE;
            resultData->result.det_result.point[detect_num].point.rectPoint.left = left;
            resultData->result.det_result.point[detect_num].point.rectPoint.top = top;
            resultData->result.det_result.point[detect_num].point.rectPoint.right = right;
            resultData->result.det_result.point[detect_num].point.rectPoint.bottom = bot;
            resultData->result.det_result.result_name[detect_num].lable_id = classId;
            sprintf(resultData->result.det_result.result_name[detect_num].lable_name, "%s: %.0f%% ", names[classId], prob*100);

            detect_num ++;
        }
    }
    resultData->result.det_result.detect_num = detect_num;
}
