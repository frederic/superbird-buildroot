...
#define BUFFER_FORMAT DRM_FORMAT_NV12
...
{
	static const int general_attribs = 5;
	...
	attribs[atti++] = EGL_ITU_REC2020_EXT; // EGL_ITU_REC601_EXT , EGL_ITU_REC709_EXT , // EGL_ITU_REC2020_EXT
	attribs[atti++] = EGL_SAMPLE_RANGE_HINT_EXT;
	attribs[atti++] = EGL_YUV_NARROW_RANGE_EXT; // EGL_YUV_NARROW_RANGE_EXT , // EGL_YUV_FULL_RANGE_EXT creates a "washed out" picture
...
#undef ADD_PLANE_ATTRIBS
	...
	glBindTexture(GL_TEXTURE_EXTERNAL_OES, buffer->gl_texture);
	...
	display->egl.image_target_texture_2d(GL_TEXTURE_EXTERNAL_OES, buffer->egl_image);

	glGenFramebuffers(1, &buffer->gl_fbo);
	glBindFramebuffer(GL_FRAMEBUFFER, buffer->gl_fbo);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
			       GL_TEXTURE_EXTERNAL_OES, buffer->gl_texture, 0);
		     ...
		     int width, int height)
{
	...
	buffer->strides[0] = (buffer->width + 31)/32 * 32;
	...
	//	buffer->strides[1] = gbm_bo_get_stride(buffer->bo);
	buffer->dmabuf_fds[1] = gbm_bo_get_fd(buffer->bo);
	//  buffer->offsets[1] = gbm_bo_get_offset(buffer->bo, 1);
	buffer->offsets[1] = buffer->strides[0] * buffer->height;

	fprintf(stderr, "\e[34;42m[Lingjie]===>%s%d offset[1] %d_ in [%s]%s:%d\e[0m\n", "offset[0]" , buffer->offsets[0],buffer->offsets[1], __func__ , __FILE__, __LINE__);
	fprintf(stderr, "\e[34;42m[Lingjie]===>%s%d strides[1] %d_ in [%s]%s:%d\e[0m\n", "strides[0]" , buffer->strides[0],buffer->strides[1], __func__ , __FILE__, __LINE__);
