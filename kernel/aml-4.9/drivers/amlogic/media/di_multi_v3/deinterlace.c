...
int dimv3_get_canvas(void)
{
	...
	if (extv3_ops.canvas_pool_alloc_canvas_table("di_pre",
						   &di_pre_idx[0][0],
						   pre_num,
						   CANVAS_MAP_TYPE_1)) {
	...
	if (extv3_ops.canvas_pool_alloc_canvas_table("di_post",
						   &di_post_idx[0][0],
						   post_num,
						   CANVAS_MAP_TYPE_1)) {
...
#endif
#ifdef CONFIG_AMLOGIC_MEDIA_MULTI_DEC
	if (extv3_ops.canvas_pool_alloc_canvas_table("di_inp",
	...
	if (extv3_ops.canvas_pool_alloc_canvas_table("di_wr",
						   &di_wr_idx, 1,
						   CANVAS_MAP_TYPE_1)) {
			  ...
			  struct di_win_s *win)
{
	...
	/* Brian: just wr directly, no endian wrap */
	smif->cbcr_swap = 0;
	#ifdef HIS_V3
		     ...
		     vframe_t *disp_vf)
{
	...
	if (dimp_get(eDI_MP_post_wr_en) && dimp_get(eDI_MP_post_wr_support)) {
		...
		if (pst->vf_post.type & VIDTYPE_VIU_NV12) {
			pst->vf_post.canvas0_config[0].endian = 0;
			pst->vf_post.canvas0_config[1].endian = 0;
		}
