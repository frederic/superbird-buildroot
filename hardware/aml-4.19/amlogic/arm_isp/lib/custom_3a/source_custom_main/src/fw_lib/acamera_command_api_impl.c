...
static uint8_t isp_safe_start( uint32_t base )
{
    ...
    while ( acamera_isp_input_port_mode_status_read( base ) != ACAMERA_ISP_INPUT_PORT_MODE_REQUEST_SAFE_START ) {
        ...
        count++;
        if ( count > 50 ) {
            LOG( LOG_DEBUG, "start fsm_mgr failed, timeout: %u.", (unsigned int)count * sleep_in_us );
            rc = 1;
            break;
        }
    }
...
uint8_t ae_frame( acamera_fsm_mgr_t *instance, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
	acamera_fsm_mgr_set_param( instance, FSM_PARAM_SET_AE_FRAMEID, &value, 0);

	return 0;
}

uint8_t iridix_frame( acamera_fsm_mgr_t *instance, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
	acamera_fsm_mgr_set_param( instance, FSM_PARAM_SET_IRIDIX_FRAMEID, &value, 0);

	return 0;
}

uint8_t gamma_frame( acamera_fsm_mgr_t *instance, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
	acamera_fsm_mgr_set_param( instance, FSM_PARAM_SET_GAMMA_FRAMEID, &value, 0);

	return 0;
}

uint8_t awb_frame( acamera_fsm_mgr_t *instance, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
	acamera_fsm_mgr_set_param( instance, FSM_PARAM_SET_AWB_FRAMEID, &value, 0);
