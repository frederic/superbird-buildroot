...
static void sensor_alloc_integration_time( void *ctx, uint16_t *int_time_S, uint16_t *int_time_M, uint16_t *int_time_L )
{
    ...
    case WDR_MODE_LINEAR: // Normal mode
        ...
        uint16_t shr0 = p_ctx->vmax - *int_time_S - 1;
        if ( p_ctx->shr0 != shr0 ) {
            p_ctx->int_cnt = 2;
            p_ctx->shr0 = shr0;
    ...
    case WDR_MODE_FS_LIN: // DOL3 Frames
        ...
        if ( p_ctx->int_time_S != *int_time_S || p_ctx->int_time_L != *int_time_L ) {
            ...
            p_ctx->shr0 = p_ctx->frame - *int_time_L;
            if ( p_ctx->shr0 < 22 ) p_ctx->shr0 = 22;
            p_ctx->shr1 = p_ctx->rhs1 - *int_time_S;
            if ( p_ctx->shr1 < 18 ) p_ctx->shr1 = 18;
        }
#endif
        break;
    }
    ...
    if ( p_ctx->int_cnt || p_ctx->gain_cnt || p_ctx->dgain_change) {
        ...
        if ( p_ctx->int_cnt ) {
            ...
            case WDR_MODE_LINEAR:
                acamera_sbus_write_u8( p_sbus, 0x3059, ( p_ctx->shr0 >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3058, ( p_ctx->shr0 >> 0 ) & 0xFF );
                break;
            case WDR_MODE_FS_LIN:
                p_ctx->shr1_old = p_ctx->shr1;
                p_ctx->shr0_old = p_ctx->shr0;
#ifdef FS_LIN_3DOL
                // SHR2
                acamera_sbus_write_u8( p_sbus, 0x3061, ( p_ctx->shr2 >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3060, ( p_ctx->shr2 >> 0 ) & 0xFF );
#endif
                // SHR0
                acamera_sbus_write_u8( p_sbus, 0x3059, ( p_ctx->shr0_old >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3058, ( p_ctx->shr0_old >> 0 ) & 0xFF );

                // SHR1
                acamera_sbus_write_u8( p_sbus, 0x305d, ( p_ctx->shr1_old >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x305c, ( p_ctx->shr1_old >> 0 ) & 0xFF );
                break;
            }
        }
...
static uint16_t sensor_get_id( void *ctx )
{
    ...
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0x3057) << 8;
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0x3056);

    if ((sensor_id != SENSOR_CHIP_ID) && (sensor_id != 0x847) && (sensor_id != 0x43C)) {
        LOG(LOG_CRIT, "%s: Failed to read sensor imx335 id, %d\n", __func__, sensor_id);
        return 0xFFFF;
    }

    LOG(LOG_CRIT, "%s: success to read sensor imx335: %x\n", __func__, sensor_id);
...
//sensor_load_sequence( p_sbus, p_ctx->seq_width, p_sensor_data, SENSOR_IMX335_SEQUENCE_DEFAULT_WDR_1080P_2DOL );
#if FS_LIN_1080P
            sensor_load_sequence( p_sbus, p_ctx->seq_width, p_sensor_data, setting_num);
#else
    ...
    if ( (param->modes_table[mode].exposures == 1) && (param->modes_table[mode].fps == 25 * 256) ) {
        acamera_sbus_write_u8( p_sbus, 0x3030, 0x18 ); //0xa8c = 2700
        acamera_sbus_write_u8( p_sbus, 0x3031, 0x15 );
        p_ctx->s_fps = 25;
        p_ctx->vmax = 5400; // VTS *30/25
    } else if ((param->modes_table[mode].exposures == 2) && (param->modes_table[mode].fps == 25 * 256)
        && (param->modes_table[mode].resolution.width == 1920)) {
        acamera_sbus_write_u8( p_sbus, 0x3030, 0xdc );//0x5dc = 1500
        acamera_sbus_write_u8( p_sbus, 0x3031, 0x05 );
        p_ctx->s_fps = 25;
        p_ctx->vmax = 1500;// VTS *30/25
    } else if ((param->modes_table[mode].exposures == 2) && (param->modes_table[mode].fps == 25 * 256)
        && (param->modes_table[mode].resolution.width == 2560)) {
        acamera_sbus_write_u8( p_sbus, 0x3030, 0x18 );
        acamera_sbus_write_u8( p_sbus, 0x3031, 0x15 );
        p_ctx->s_fps = 25;
        p_ctx->vmax = 5400;
    } else {
        p_ctx->vmax = ((uint32_t)acamera_sbus_read_u8(p_sbus,0x3031) << 8) |acamera_sbus_read_u8(p_sbus,0x3030);
    }

    int8_t r = acamera_sbus_read_u8( p_sbus, 0x3018 );
    switch ( r ) {
    case 0:
        param->active.width = 2560;
        param->active.height = 1944;
    ...
    // pixel_clock = HTS * VTS * FPS
    if ( param->modes_table[mode].wdr_mode == WDR_MODE_FS_LIN ) {
            p_ctx->pixel_clock = 37125000;
        }

    param->total.width = (( (uint16_t)acamera_sbus_read_u8( p_sbus, 0x3034 ) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x3035 ));
...
static void start_streaming( void *ctx )
{
    ...
    // trigger auto calibration process for the imx334 sensor
...
*/
    ...
    s_ctx.param.isp_context_seq.seq_num= SENSOR_IMX335_CONTEXT_SEQ;
