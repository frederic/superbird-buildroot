...
#define AGAIN_MAX_DB 0x64  // (30 * 10/3)
...
static void sensor_alloc_integration_time( void *ctx, uint16_t *int_time_S, uint16_t *int_time_M, uint16_t *int_time_L )
{
    ...
    case WDR_MODE_FS_LIN: // DOL3 Frames
        ...
        if ( p_ctx->int_time_S != *int_time_S || p_ctx->int_time_L != *int_time_L ) {
            ...
            p_ctx->shr0 = p_ctx->frame - *int_time_L - 1;
            if ( p_ctx->shr0 < 20 ) p_ctx->shr0 = 20;
            p_ctx->shr1 = p_ctx->rhs1 - *int_time_S - 1;
            if ( p_ctx->shr1 < 9 ) p_ctx->shr1 = 9;
        }
#endif
        break;
    }
...
static uint16_t sensor_get_id( void *ctx )
{
    ...
    LOG(LOG_CRIT, "%s: success to read sensor imx334: %x\n", __func__, sensor_id);
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    if ( (param->modes_table[mode].exposures == 1) && (param->modes_table[mode].fps == 25 * 256) ) {
        acamera_sbus_write_u8( p_sbus, 0x3030, 0x8c ); //0xa8c = 2700
        acamera_sbus_write_u8( p_sbus, 0x3031, 0x0a );
        p_ctx->s_fps = 25;
        p_ctx->vmax = 2700; // VTS *30/25
        ...
        && (param->modes_table[mode].resolution.width == 3840)) {
        acamera_sbus_write_u8( p_sbus, 0x3030, 0x91 );//0x5dc = 2705
        acamera_sbus_write_u8( p_sbus, 0x3031, 0x0a );
        p_ctx->s_fps = 25;
        p_ctx->vmax = 2705;
    } else {
    ...
    if ( param->modes_table[mode].wdr_mode == WDR_MODE_FS_LIN ) {
        if ( param->modes_table[mode].resolution.width == 1920 ) {
            p_ctx->pixel_clock = 37125000;
        }else {
            p_ctx->pixel_clock = 30023280;
        }
    }
...
void sensor_init_imx334( void **ctx, sensor_control_t *ctrl, void* sbp)
{
    ...
    *ctx = &s_ctx;
    ...
    s_ctx.param.isp_context_seq.seq_num= SENSOR_IMX334_ISP_CONTEXT_SEQ;
