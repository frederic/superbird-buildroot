...
AMVEnc_Status initEncParams(AMVHEVCEncHandle *handle, vl_encode_info_hevc_t encode_info) {
    ...
    handle->mEncParams.prepend_spspps_to_idr_frames =
...
vl_codec_handle_hevc_t vl_video_encoder_init_hevc(vl_codec_id_hevc_t codec_id, vl_encode_info_hevc_t encode_info) {
    AMVEnc_Status ret;
    ...
    ret = initEncParams(mHandle, encode_info);
...
int vl_video_encoder_encode_hevc(vl_codec_handle_hevc_t codec_handle, vl_frame_type_hevc_t frame_type, unsigned int outputBufferLen, unsigned char* out, vl_buffer_info_hevc_t* buffer_info) {
    AMVEnc_Status ret;
