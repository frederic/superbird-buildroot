...
AMVEnc_Status AMRCInitFrameQP(amvenc_hw_t *hw_info, bool IDR, int bitrate, float frame_rate, uint32 continuousSkipCnt)
{
    AMVEnc_Status ret = AMVENC_FAIL;
    if (!hw_info)
        return AMVENC_MEMORY_FAIL;
    ...
    if (grc[hw_info->dev_id]->InitFrameQP != NULL)
        ret = grc[hw_info->dev_id]->InitFrameQP(hw_info->dev_data, hw_info->rc_data, IDR, bitrate, frame_rate, continuousSkipCnt);
    return ret;
}
