...
bypass_active_keep_alive:
...
#endif /* CONFIG_ACTIVE_KEEP_ALIVE_CHECK */
			{
				if (rx_chk != _SUCCESS) {
					if (pmlmeext->retry == 0) {
#ifdef DBG_EXPIRATION_CHK
						RTW_INFO("issue_probereq(ssid=%s) to trigger probersp, retry=%d, from_timer=%d\n", pmlmeinfo->network.Ssid.Ssid, pmlmeext->retry, from_timer);
#endif
					/* from_timer=1 - driver in LPS, driver cannot wait here!
					from_timer=0  - driver wait ACK to avoid probe-req(pw bit=0) sent and enter LPS simultaneously!
									(probe-req(pw bit=0) could be sent out after LPS null(1) sent already)*/
...
#endif
					/* from_timer=1 - driver in LPS, driver sent null(1) and cannot wait here!
					from_timer=0  - driver wait ACK to avoid null(0) sent and enter LPS simultaneously!
									(null(0) could be sent out after LPS null(1) sent already) */
					if (from_timer) {
