...
void UpdateHalRAMask8723B(PADAPTER padapter, struct sta_info *psta, u32 mac_id, u8 rssi_level)
{
...
#if 1
	phydm_update_hal_ra_mask(&pHalData->odmpriv, psta->wireless_mode, pHalData->rf_type, bw, MimoPs_enable, disable_cck_rate, &ratr_bitmap_msb, &mask, rssi_level);
#else
	...
	if (pHalData->fw_ractrl == _TRUE)
		rtl8723b_set_FwMacIdConfig_cmd(padapter, mac_id, psta->raid, bw, shortGIrate, mask);
...
void init_hal_spec_8723b(_adapter *adapter)
{
	...
	hal_spec->port_num = HW_PORT_NUM_8723B;
...
static void hw_var_set_opmode(PADAPTER padapter, u8 variable, u8 *val)
{
...
#endif /* CONFIG_CONCURRENT_MODE */
	{
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
			...
			rtw_write16(padapter, REG_BCNTCFG, 0x00);
			rtw_write16(padapter, REG_TBTT_PROHIBIT, 0x8004);
			rtw_write16(padapter, REG_TSFTR_SYN_OFFSET, 0x7fff);/* +32767 (~32ms) */
...
static void hw_var_set_mlme_join(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	if (type == 0) {
		...
		else if ((rtw_mi_get_ap_num(padapter) == 1) && (rtw_mi_get_assoced_sta_num(padapter) == 1))
