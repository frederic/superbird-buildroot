		      ...
		      IN mlan_ioctl_req *pioctl_buf)
{
	...
	if (resp->result == HostCmd_RESULT_OK) {
		...
		if (header->type == TLV_TYPE_ROAM) {
			...
			pevent->event_id =
				MLAN_EVENT_ID_DRV_ROAM_OFFLOAD_ENABLE;
			pevent->event_len =
				sizeof(roam_offload_enable->roam_enable);
			memcpy(pmpriv->adapter, (t_u8 *)pevent->event_buf,
			       (t_u8 *)&roam_offload_enable->roam_enable,
			       MIN(sizeof(event_buf),
				   sizeof(roam_offload_enable->roam_enable)));
		/**Inform uplayer to add support flag**/
